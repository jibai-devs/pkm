...
#include "mechanics/SelectProc.h"
...
inline void SelectDamageCounterAny(State &state) {
  state.setSelect(SelectType::Card, SelectContext::DamageCounterAny,
                  state.effectPlayerIndex());
  for (const AreaRef &ref : state.targetList) {
    Card *card = state.checkGetCard(ref);
    if (card != nullptr) {
      AddOptionCard(state, card->area, state.currentAreaIndex(ref.card, *card),
                    card->playerIndex);
    }
  }
...
inline void SelectedAttackDamageChangePutDamageCounter(State &state) {
  const int count = state.selectedNumber();
  state.clearSelect();
...
inline void SelectedAddDamageCounterSwitchAny(State &state,
...
inline void SelectedRemoveDamageCounterSwitchAny(State &state,
                                                 int targetPlayerIndex) {
...
inline void DamageCounterSwitchAny(State &state, int targetPlayerIndex) {
  ...
  state.setSelect(SelectType::Card, SelectContext::RemoveDamageCounter,
                  state.effectPlayerIndex(), 0, 1);
...
inline void AfterAttackDamageChangeTargetCountEnemyCoin(State &state) {
  state.attackDamageChange =
      state.getEffectValue() *
      (static_cast<int>(state.targetList.size()) - state.coinHeadCount);
}
...
inline void SelectedSpecialCondition(State &state) {
  ...
  state.clearSelect();
  const auto type = static_cast<SelectSpecialConditionType>(o.param0);
  const int playerIndex = 1 - state.effectPlayerIndex();
  switch (type) {
...
inline void SelectedRecoverSpecialCondition(State &state, int playerIndex) {
  SelectOption o = state.firstSelected();
  state.clearSelect();
  SelectSpecialConditionType type =
      static_cast<SelectSpecialConditionType>(o.param0);
  ...
  for (const AreaRef &preRef : state.targetList) {
    ...
    if (c != nullptr) {
      ...
      if (preCard.area == AreaType::Active || preCard.area == AreaType::Bench) {
        ...
        const int areaIndex = state.currentAreaIndex(ref, afterCard);
        CardRef afterRef = state.removeCardRef(afterCard.area, areaIndex,
                                               afterCard.playerIndex);
        assert(afterRef == ref);
        const int ownerAreaIndex = state.currentAreaIndex(preRef.card, preCard);
        CardRef &inPlayRef =
            (preCard.area == AreaType::Active ? ps.active.at(0)
                                              : ps.bench.at(ownerAreaIndex));
...
inline void EffectAttackDamage(State &state, int cardIndex, int baseDamage) {
  ...
  const Card &effectCard = state.getCard(effectCardRef);
  int damage =
      CalcDamage(state, baseDamage, ref, card, effectCardRef, effectCard,
                 card.area == AreaType::Active, &state.getAttack());
  state.changed = true;
...
inline void SelectedDamageMultiAll(State &state) {
  const int playerIndex = 1 - state.effectPlayerIndex();
  for (int i = static_cast<int>(state.selectCounts.size()) - 1; i >= 0; i--) {
    int count = state.selectCounts[i];
    if (count > 0) {
      const int baseDamage = state.getSecondEffectValue() * count;
      const AreaType area = (i == 0 ? AreaType::Active : AreaType::Bench);
      const int areaIndex = (i == 0 ? 0 : i - 1);
      const CardRef ref = state.getCardRef(area, areaIndex, playerIndex);
...
inline void SelectDamageMulti(State &state) {
  state.setSelect(SelectType::Card, SelectContext::Damage,
                  state.effectPlayerIndex());
  for (const AreaRef &ref : state.targetList) {
...
inline void SelectedDisableAttack(State &state, int cardIndex) {
  const int attackId = state.firstSelected().param0;
  state.clearSelect();
  CardRef ref(cardIndex);
...
inline void AfterDeckToTrashCoinUntilTail(State &state) {
  const int count = state.coinHeadCount;
  ...
  for (int i : state.effectTargetPlayer(state.getEffect())) {
    ...
    for (int _ : range(count)) {
      ...
      state.changed = true;
      CardRef ref = MoveCard(state, i, AreaType::Deck, ps.deck.size() - 1,
                             AreaType::Trash);
...
inline void EffectInstatnt(State &state) {
  ...
  case EffectType::ToDeckBottomClose:
    if (state.targetList.size() >= 2) {
      if (state.game->config.deviceRand) {
        std::shuffle(state.targetList.begin(), state.targetList.end(),
                     std::random_device());
      } else {
        std::shuffle(state.targetList.begin(), state.targetList.end(),
                     state.game->rng);
  ...
  case EffectType::ToActiveAndTrashActive: {
    ...
    if (card != nullptr) {
      ...
      if (!ps.active.empty()) {
        MoveCard(state, card->playerIndex, AreaType::Active, 0,
                 AreaType::Trash);
      }
  ...
  case EffectType::ToLooking:
    ...
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        state.changed = true;
        MoveRefCard(state, ref.card, *card, AreaType::Looking,
                    state.lookingOpenType());
      }
    }
    break;
  ...
  case EffectType::Switch: {
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        ...
        } else if (effect.effectTargetActive ||
                   (state.effectPlayerIndex() != card->playerIndex &&
                    state.selectPlayer != card->playerIndex)) {
  ...
  case EffectType::LookDeck: {
    ...
    for (int i = 0; i < count; i++) {
      ...
      MoveCard(state, playerIndex, AreaType::Deck, index, AreaType::Looking,
               state.lookingOpenType());
    }
    break;
  }
  case EffectType::LookDeckReverse: {
    assert(state.looking.empty());
    int count = state.getEffectValue();
    const int playerIndex = state.getEffectTargetPlayerIndex();
    assert(playerIndex <= 1);
  ...
  case EffectType::LookDeckBottom: {
    ...
    for (int i = 0; i < count; i++) {
      state.changed = true;
      MoveCard(state, playerIndex, AreaType::Deck, 0, AreaType::Looking,
               state.lookingOpenType());
    }
    break;
  }
  case EffectType::LookAndReturn: {
    ...
    for (const AreaRef &ref : state.targetList) {
      ...
      LogMoveCard(state, pos.playerIndex, ref.card, pos.area, AreaType::Looking,
                  openType);
    }
    ...
    for (const AreaRef &ref : state.targetList) {
      CardPosition pos = state.currentCardPosition(ref.card);
      LogMoveCard(state, pos.playerIndex, ref.card, AreaType::Looking, pos.area,
                  openType);
      if (state.getEffectValue() == 1) {
  ...
  case EffectType::DamageCounter:
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        int damage =
            state.getEffectValue() * 10 + state.effectState.damageChange;
        if (state.isPreventEffect(*card)) {
          damage = 0;
        }
        ...
        AddDamage(state, ref.card, *card, damage, false,
                  state.getEffectCard().card, true);
        if (damage > 0) {
          state.changed = true;
        }
      }
  ...
  case EffectType::DamageCounterAny: {
    ...
    const int count = state.getEffectValue();
    if (count <= 0) {
      break;
    }
  ...
  case EffectType::DamageCounterTypeEnergyCountMe:
    for (const AreaRef &ref : state.targetList) {
      Card *card = state.checkGetCard(ref);
      if (card != nullptr) {
        int count = state.typeEnergyCount(
            state.effectPlayerIndex(), state.getEffectCard().card,
            static_cast<EnergyType>(state.getEffectValue()));
  ...
  case EffectType::AttackDamage:
    ...
    for (int i = static_cast<int>(state.targetList.size()) - 1; i >= 0; i--) {
      const AreaRef &ref = state.targetList[i];
      Card *card = state.checkGetCard(ref);
      if (card != nullptr) {
        const int baseDamage =
            state.getEffectValue() + state.effectState.damageChange;
    ...
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        ...
        if (maxCount >= 2) {
          ...
          state.setSelect(SelectType::Count,
                          SelectContext::RemoveDamageCounterCount,
                          state.effectPlayerIndex());
  ...
  case EffectType::Devolve: {
    const auto toArea = static_cast<AreaType>(state.getEffectValue());
    for (const AreaRef &ref : state.targetList) {
      Card *card = state.checkGetCardNotPreventEffect(ref);
      if (card != nullptr) {
  ...
  case EffectType::DevolveAny: {
    ...
    const AreaRef preRef = state.targetList[0];
    Card *card = state.checkGetCardNotPreventEffect(preRef);
    if (card == nullptr) {
      break;
    }
    const CardPosition pos = state.currentCardPosition(preRef.card);
    ...
    if (state.isEvolved(ref)) {
      ...
      state.setSelect(SelectType::YesNo, SelectContext::MoreDevolve,
                      state.effectPlayerIndex());
  ...
  case EffectType::EvolvesToEach:
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        ...
        CardPosition pos = state.currentCardPosition(ref);
        EvolveProc(state, pos.area, pos.areaIndex, contextCard.area, index,
                   pos.playerIndex);
      }
    }
    break;
  ...
  case EffectType::EvolvesFromEach:
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        ...
        CardPosition pos = state.currentCardPosition(ref);
        EvolveProc(state, contextPos.area, contextPos.areaIndex, pos.area,
                   pos.areaIndex, pos.playerIndex);
        break;
      }
    }
    break;
    ...
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        ...
        CardRef targetRef =
            state.selectedList.at(state.effectState.selectedListIndex);
        CardPosition pos = state.currentCardPosition(ref.card);
        AttachProc(state, pos.area, pos.areaIndex, pos.playerIndex, targetRef,
                   true);
  ...
  case EffectType::AttachEnergyMe:
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        ...
        CardRef effectCardRef = state.getEffectCard().card;
        CardPosition pos = state.currentCardPosition(ref.card);
        AttachProc(state, pos.area, pos.areaIndex, pos.playerIndex,
                   effectCardRef, true);
      }
    }
    break;
    ...
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        for (CardRef attachRef : state.selectedList) {
          ...
          AttachProc(state, pos.area, pos.areaIndex, pos.playerIndex, ref.card,
                     true);
    ...
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        for (CardRef energyRef : state.selectedList) {
          state.changed = true;
          if (state.isPreventEffect(*card)) {
            MoveRefCard(state, energyRef, state.getCard(energyRef),
                        AreaType::Trash);
          } else {
  ...
  case EffectType::DelayEffect:
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        ...
        ta.activateInfo = ActivateAbilityInfo(delay->skillId, effectCardRef,
                                              effectCard.playerIndex);
  ...
  case EffectType::AttackDamageChangeTargetCount:
    state.attackDamageChange =
        state.getEffectValue() * static_cast<int>(state.targetList.size());
    break;
  case EffectType::EffectDamageChangeTargetCount:
    state.effectState.damageChange =
        state.getEffectValue() * static_cast<int>(state.targetList.size());
    break;
  case EffectType::AttackDamageChangeEnergyCount:
    state.attackDamageChange =
        state.getEffectValue() * state.targetListEnergyCount();
    break;
  ...
  case EffectType::AttackDamageChangeTypeEnergyCount: {
    const int count = state.targetListTypeEnergyCount(
        static_cast<EnergyType>(state.getEffectValue()));
  ...
  case EffectType::AttackDamageChangeTargetCountCoin:
    state.pushFunction(AfterAttackDamageChangeCoinUntilTail);
    SelectCoin(state, state.effectPlayerIndex(),
               static_cast<int>(state.targetList.size()));
    break;
  case EffectType::AttackDamageChangeTargetCountEnemyCoin:
    state.pushFunction(AfterAttackDamageChangeTargetCountEnemyCoin);
    SelectCoin(state, 1 - state.effectPlayerIndex(),
               static_cast<int>(state.targetList.size()));
    break;
  ...
  case EffectType::EffectDamageChangeDamageCounter:
    ...
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        state.effectState.damageChange +=
            state.getEffectValue() * card->damage / 10;
      }
    }
    break;
  case EffectType::AttackDamageChangePutDamageCounter:
    state.setSelect(SelectType::Count, SelectContext::DamageCounterCount,
                    state.effectPlayerIndex());
  ...
  case EffectType::AttackDamageChangeTypeCount: {
    ...
    for (const AreaRef &ref : state.targetList) {
      ...
      type |= static_cast<unsigned>(card.getMaster().energyType);
    }
  ...
  case EffectType::AttackDamageChangePreTurnTakePrizeCount:
    state.attackDamageChange = state.getEffectValue() *
                               state.turnHistories[1].takePrizeCountTurnPlayer;
    break;
  ...
  case EffectType::RecoverSpecialConditionSingle:
    for (int i : state.effectTargetPlayer(effect)) {
      ...
      const PlayerState &ps = state.players[i];
      state.changed = true;
      state.setSelect(SelectType::SpecialCondition,
                      SelectContext::RecoverSpecialCondition,
                      state.effectPlayerIndex());
      if (ps.isPoisoned()) {
  ...
  case EffectType::DrawTargetCount:
    for (int i : state.effectTargetPlayer(effect)) {
      Draw(state, i, static_cast<int>(state.targetList.size()));
    }
    break;
  ...
  case EffectType::DrawUntilPsychic:
    for (int i : state.effectTargetPlayer(effect)) {
      ...
      for (RefPosition &rp : inPlay) {
        ...
        if (ContainsEnergyType(state.getEnergyType(card),
                               EnergyType::Psychic)) {
          count++;
        }
      }
  ...
  case EffectType::DrawMirror:
    for (int i : state.effectTargetPlayer(effect)) {
      Draw(state, i,
           state.players[1 - i].hand.size() - state.players[i].hand.size());
    }
    break;
  ...
  case EffectType::TakePrizeCountChangeTerastalAttackKoActive:
    for (int i : state.effectTargetPlayer(effect)) {
      state.players[i].takePrizeCountChangeTerastalAttackKoActive +=
          state.getEffectValue();
    }
    break;
  case EffectType::TakePrizeCountChangeNAttackKoActive:
    for (int i : state.effectTargetPlayer(effect)) {
      state.players[i].takePrizeCountChangeNAttackKoActive +=
          state.getEffectValue();
    }
    break;
  ...
  case EffectType::CannotUseSelectedAttack:
    for (const AreaRef &ref : state.targetList) {
      ...
      if (card != nullptr) {
        ...
        if (!master.attacks.empty()) {
          state.setSelect(SelectType::Attack, SelectContext::DisableAttack,
                          state.effectPlayerIndex());
  ...
  case EffectType::MetalDamageChangeNextTurn:
    for (int i : state.effectTargetPlayer(effect)) {
      ClampShort(state.players[i].nextTurn.metalDamageChange,
                 state.getEffectValue());
    }
    break;
  ...
  case EffectType::SelectActivate:
    state.setSelect(SelectType::YesNo, SelectContext::Activate,
                    state.effectPlayerIndex());
    AddOptionYesAndNo(state);
    state.pushFunction(SelectedActivate);
    break;
  case EffectType::SelectEffect:
    state.setSelect(SelectType::YesNo, SelectContext::FirstEffect,
                    state.effectPlayerIndex());
    AddOptionYesAndNo(state);
  ...
  case EffectType::SkipIfCoinTail:
    ...
    SelectCoin(state,
               state.getSecondEffectValue() == 1
                   ? (1 - state.effectPlayerIndex())
                   : state.effectPlayerIndex(),
               1);
    break;
  ...
  case EffectType::SelectPoisonBurnConfuse:
    state.setSelect(SelectType::SpecialCondition,
                    SelectContext::AffectSpecialCondition,
                    state.effectPlayerIndex());
    AddOptionSpecialCondition(state, SelectSpecialConditionType::Poison);
    AddOptionSpecialCondition(state, SelectSpecialConditionType::Burn);
    AddOptionSpecialCondition(state, SelectSpecialConditionType::Confuse);
