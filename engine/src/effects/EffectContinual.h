...
inline bool IsNoEffectActive(const State &state, int playerIndex,
                             CardRef effectCardRef) {
...
inline void EffectContinual(State &state, const Effect &effect,
                            const std::vector<AreaRef> &targetList,
                            int effectPlayerIndex, CardRef effectCardRef) {
  ...
  case EffectType::DamageChangeEnemyTakenPrize:
    for (const AreaRef &ref : targetList) {
      Card &card = state.getCard(ref.card);
      ClampShort(card.damageChangeEnemyTakenPrize,
                 state.getValueByEffect(effect));
    }
    break;
  ...
  case EffectType::TakeEnemyAttackDamageChange:
    for (const AreaRef &ref : targetList) {
      Card &card = state.getCard(ref.card);
      ClampShort(card.takeEnemyAttackDamageChange,
                 state.getValueByEffect(effect));
    }
    break;
  case EffectType::TakeEnemyAbilityPokemonAttackDamageChange:
    for (const AreaRef &ref : targetList) {
      Card &card = state.getCard(ref.card);
      ClampShort(card.takeEnemyAbilityPokemonAttackDamageChange,
                 state.getValueByEffect(effect));
    }
    break;
  case EffectType::TakeEnemyFireOrWaterPokemonAttackDamageChange:
    for (const AreaRef &ref : targetList) {
      Card &card = state.getCard(ref.card);
      ClampShort(card.takeEnemyFireOrWaterPokemonAttackDamageChange,
                 state.getValueByEffect(effect));
    }
    break;
  case EffectType::TakeEnemy4TypePokemonAttackDamageChange:
    for (const AreaRef &ref : targetList) {
      Card &card = state.getCard(ref.card);
      ClampShort(card.takeEnemy4TypePokemonAttackDamageChange,
                 state.getValueByEffect(effect));
    }
    break;
  ...
  case EffectType::RetreatCostChange:
    for (const AreaRef &ref : targetList) {
      Card &card = state.getCard(ref.card);
      const int val = card.retreatCostChange + state.getValueByEffect(effect);
  ...
  case EffectType::AttackCostChangeColorless:
    for (const AreaRef &ref : targetList) {
      Card &card = state.getCard(ref.card);
      const int val =
          card.attackCostChangeColorless + state.getValueByEffect(effect);
  ...
  case EffectType::AttackCostDown:
    for (const AreaRef &ref : targetList) {
      Card &card = state.getCard(ref.card);
      const int val = card.attackCostDown + state.getValueByEffect(effect);
  ...
  case EffectType::AttackCostDownColorlessTargetCount: {
    ...
    const int val =
        card.attackCostChangeColorless - static_cast<int>(targetList.size());
    card.attackCostChangeColorless = std::clamp(val, -100, 100);
  } break;
  case EffectType::AttackCostDownColorlessOwnAttackEnemyTakenPrize:
    for (const AreaRef &ref : targetList) {
      Card &card = state.getCard(ref.card);
      const int prize = state.players.at(1 - effectPlayerIndex).prize.size();
      const int val =
          card.attackCostDownColorlessOwnAttack + PRIZE_SIZE - prize;
  ...
  case EffectType::AddEnergyType:
    for (const AreaRef &ref : targetList) {
      Card &card = state.getCard(ref.card);
      const int val = state.getValueByEffect(effect);
      card.typeIndex = val;
    }
    break;
  ...
  case EffectType::NoAbility:
    for (const AreaRef &ref : targetList) {
      ...
      for (int i = 0; i < state.currentCardEffectIndex; i++) {
        ...
        if (ce.ref == ref.card) {
          card.skillOrder = state
                                .getCard(state.game->cardEffectList
                                             .at(state.currentCardEffectIndex)
                                             .ref)
                                .skillOrder +
                            1;
