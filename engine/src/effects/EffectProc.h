...
#include "effects/EffectContinual.h"
#include "effects/EffectInstant.h"
...
inline void ParalyzeProc(State &state) {
  const int playerIndex = state.activePlayerIndex();
...
inline void BurnProc(State &state, int playerIndex) {
  ...
  if (ps.burned) {
    ...
    const int damage = 20 + ps.burnDamageChange;
...
inline void SpecialConditionProc(State &state) {
  for (int i : state.basicPlayerOrder()) {
    ...
    if (ps.isPoisoned()) {
      ...
      if (!ContainsEnergyType(state.getEnergyType(state.getCard(ref)),
                              EnergyType::Darkness)) {
...
inline void StaticEffect(State &state, CardRef ref) {
  ...
  const int skillOrder = card.skillOrder;
  ...
  if (skill.notStack &&
      state.game->abilitySet[card.playerIndex].contains(skill.skillId)) {
    return;
  }
  ...
  const AreaRef areaRef = state.makeAreaRef(ref);
  for (int i = 0; i < static_cast<int>(skill.effects.size()); i++) {
    ...
    for (int i = static_cast<int>(targetList.size()) - 1; i >= 0; i--) {
      const Card &c = state.getCard(targetList[i].card);
      if (c.noEnemyAbility && c.playerIndex != card.playerIndex &&
          card.area != AreaType::Stadium) {
...
inline void RefreshEffect(State &state, int depth) {
  ...
  auto func = [&state, &cardList](CardRef ref) {
    ...
    cardList.push_back(
        {ref, ability->priority, card.skillOrder, card.moveCounter});
  };
  ...
  std::sort(cardList.begin(), cardList.end(),
            [](const CardEffect &left, const CardEffect &right) {
...
inline void AfterAbility(State &state) { state.clearAbility(); }
...
inline void ActivateEffect2(State &state) {
  ...
  if (state.getEffect().loopCount > 0 && targetList.empty()) {
...
inline void SelectedEvolveTarget(State &state) {
  ...
  const SelectOption o = state.options.at(state.selected[0]);
  const int playerIndex = state.selectPlayer;
  state.clearSelect();
  state.changed = true;
  EvolveProc(state, static_cast<AreaType>(o.param0), o.param1,
             static_cast<AreaType>(o.param2), o.param3, playerIndex);
}
...
inline void SelectedCardOrAttachedCard(State &state) {
  ...
  for (int i : state.selected) {
    ...
    if (o.type == SelectOptionType::Card) {
      ...
      CardRef pokemonRef =
          state.getCardRef(pos.area, pos.areaIndex, pos.playerIndex);
      int moveCounter = state.getCard(pokemonRef).moveCounter;
      int index = 0;
      PlayerState &ps = state.players[pos.playerIndex];
      CardList &list =
          (o.type == SelectOptionType::EnergyCard ? ps.energy : ps.tool);
...
inline void ActivateEffect(State &state, const Effect &effect,
                           int effectIndex) {
  ...
  if (effect.isCondition) {
    ...
    if (effectIndex >= firstCheckIndex) {
      if (!SatisfyCondition(state, state.getEffects(), effectIndex,
                            state.getEffectCard().card,
                            state.effectPlayerIndex())) {
        if (effect.failSkip) {
  ...
  if (effect.selectTargetCount) {
    selectCount = static_cast<int>(targetList.size());
    ...
    for (const RefPosition &rp :
         state.players[playerIndex].getInPlayPokemon()) {
  ...
  if (!effect.notUpdateTarget) {
    if (effect.target.areas.empty() ||
        effect.target.areas[0] != AreaType::EffectedPreTarget) {
      state.preTargetList = targetList;
    }
    TargetList(state, effect.target, targetList, state.getEffectCard(),
               state.effectPlayerIndex());
    if (targetList.empty()) {
      ...
      if (effect.target.areas.size() == 1) {
        if (effect.target.areas[0] == AreaType::TriggerSubject ||
            effect.target.areas[0] == AreaType::TriggerObject) {
          AfterEffect(state);
          return;
        }
      }
  ...
  if (effect.effectType == EffectType::Switch) {
    ...
    if (targetPlayer.size() == 1) {
      const int index = targetPlayer[0];
      if (effect.effectTargetActive ||
          (state.effectPlayerIndex() != index && selectPlayerIndex != index)) {
  ...
  if (effect.effectSelectType == EffectSelectType::Evolve ||
      effect.effectSelectType == EffectSelectType::Evolve2) {
    auto inPlayPokemon = state.players[selectPlayerIndex].getInPlayPokemon();
    state.setSelect(SelectType::Evolve, SelectContext::Evolve,
                    selectPlayerIndex);
    for (const AreaRef &ref : targetList) {
  ...
  } else if (effect.effectSelectType == EffectSelectType::Energy ||
             effect.effectSelectType == EffectSelectType::MaxEnergyCard) {
    ...
    if (playerIndex >= 0) {
      ...
      if (effect.effectSelectType == EffectSelectType::MaxEnergyCard) {
        ...
        int selectMax =
            std::min(static_cast<int>(targetList.size()), selectCount);
        int selectMin = 1;
        ...
        state.setSelect(SelectType::AttachedCard, context, selectPlayerIndex,
                        selectMin, selectMax);
        for (const AttachedCard &ac : selectList) {
          ...
          AddOptionEnergyCard(state, target.area, target.index, playerIndex,
                              ac.attachedIndex);
        }
        ...
        SelectPokemonEnergy(state, selectPlayerIndex, selectList, cost,
                            context);
      }
    }
    ...
    for (const AttachedCard &ac : selectList) {
      ...
      AddOptionToolCard(state, target.area, target.index, toolCard.playerIndex,
                        ac.attachedIndex);
    }

    state.pushFunction(SelectedToolTarget);
  } else if (effect.effectSelectType ==
             EffectSelectType::CardOrAttachedCardCount) {
    SelectContext context = effect.selectContext;
    if (context == SelectContext::Discard) {
    ...
    const int selectMax =
        std::min(static_cast<int>(targetList.size()), selectCount);
    const int selectMin = 1;
    state.setSelect(SelectType::CardOrAttachedCard, context, selectPlayerIndex,
                    selectMin, selectMax);
    for (const AreaRef &ref : targetList) {
      const Card &card = state.getCard(ref.card);
      const int areaIndex =
          state.currentAreaIndex(card.playerIndex, card.area, ref.card);
      assert(areaIndex >= 0);
      ...
      if (card.area == AreaType::Energy) {
        ...
        for (CardRef r : list) {
          if (r == ref.card) {
            break;
          } else if (card.attachMoveCounter ==
                     state.getCard(r).attachMoveCounter) {
            index++;
          }
        }
        RefPosition target = state.attachedCardPosition(card);
        AddOptionEnergyCard(state, target.area, target.index, card.playerIndex,
                            index);
        ...
        RefPosition target = state.attachedCardPosition(card);
        AddOptionToolCard(state, target.area, target.index, card.playerIndex,
                          index);
      } else {
    ...
    if (effect.effectSelectType == EffectSelectType::CardUntil) {
      selectCount = static_cast<int>(targetList.size()) - effect.selectCount;
      if (selectCount <= 0) {
        return;
      }
    ...
    int selectMax = std::min(static_cast<int>(targetList.size()), selectCount);
    ...
    } else if (effect.effectType == EffectType::AttachSelectedCard ||
               effect.effectType == EffectType::SwitchSelectedCard) {
    ...
    if (effect.effectSelectType == EffectSelectType::MaxCardCount) {
      ...
      selectMax = static_cast<int>(targetList.size());
    }
    if (effect.canNoSelect) {
      selectMin = 0;
    }
    ...
    if (effect.randomSelect) {
      ...
      state.setSelect(SelectType::Card, effect.selectContext, selectPlayerIndex,
                      selectMin, selectMax);
...
inline void ActivateEffectEachSelected(State &state, int effectIndex) {
  ...
  state.contextCard =
      state.selectedList.at(state.effectState.selectedListIndex);
...
inline void ActivateSkillEffect(State &state) {
  const int effectIndex = state.calledCount();
...
inline void ActivateAbility2(State &state) {
  ...
  if (skill.onceTurn) {
    ...
    const CardId cardId = card.getMaster().cardId;
  ...
  state.setLastFunctionCallCount(static_cast<int>(skill.effects.size()));
...
inline void ActivateEnemySelectedEffect(State &state, int index) {
  ActivateAbility2(state);
  if (index == 1) {
    state.lastFunction().calledCount =
        state.getSkill().secondEffectStartIndexEnemy;
  }
}
...
inline void ActivateAbility(State &state) {
  ...
  if (skill.canSelectActivate) {
    ...
    SetYesNoSelect(state, SelectContext::Activate,
                   state.effectState.usePlayerIndex());
...
inline bool SatisfyFirstSkillCondition(State &state) {
  const ActivateAbilityInfo &info = state.effectState.ability;
  const Skill &skill = SkillTable.at(info.skillId);
  return SatisfySkillCondition(state, skill, info.effectCard.card,
                               info.usePlayerIndex, skill.triggerStartIndex);
}
...
inline int AreaTypeTriggerOrder(AreaType areaType) {
  return -static_cast<int>(areaType);
}
...
inline void AfterMoveTriggerStack(State &state, int depth) {
  if (state.triggerStack.size() >= 1) {
    ...
    if (ap.activateInfo.isSpecialCondition) {
      ...
      if (card.area == AreaType::Trash || card.area == AreaType::Deck ||
          card.area == AreaType::Hand || card.area == AreaType::Prize) {
...
inline void SelectedSkillOrder(State &state, int depth) {
  const int first = FirstTriggerIndex(state, depth);
  for (int i = static_cast<int>(state.selected.size()) - 1; i >= 0; i--) {
    state.triggerStack.push_back(
        state.temporaryTriggerStack.at(state.selected[i] + first));
  }
  state.clearSelect();
...
inline void ResolveTriggerStack(State &state, int depth) {
  const int first = FirstTriggerIndex(state, depth);
  const int count =
      static_cast<int>(state.temporaryTriggerStack.size()) - first;
  if (count > 0) {
    if (count >= 2) {
      ...
      if (state.phase != GamePhase::Main ||
          type == TriggerType::DamagedEnemyAttackActive ||
          type == TriggerType::DamagedEnemyAttack) {
        selectPlayer = 1 - state.activePlayerIndex();
      }

      state.setSelect(SelectType::Skill, SelectContext::SkillOrder,
                      selectPlayer, count, count);
      for (int i = first; i < std::ssize(state.temporaryTriggerStack); i++) {
        const TriggeredAbility &ta = state.temporaryTriggerStack[i];
...
inline void KOProc3(State &state) {
  for (const AreaRef &ref : state.koList) {
    ...
    if (card != nullptr) {
      ...
      const int prize = state.getPrizeCount(*card);
      if (!master.noPrize) {
      ...
      if (state.phase == GamePhase::Main) {
        if (state.isPlayerTurn() &&
            state.activePlayerIndex() != card->playerIndex) {
  ...
  for (int i = static_cast<int>(state.koList.size()) - 1; i >= 0; i--) {
    ...
    if (card != nullptr) {
      int playerIndex = card->playerIndex;
      const int index =
          state.currentAreaIndex(playerIndex, card->area, ref.card);
      AreaType toArea = AreaType::Trash;
      if (card->koByDamageToHand && card->koEnemyAttackDamage) {
        toArea = AreaType::Hand;
      }
      MoveCard(state, playerIndex, card->area, index, toArea, 0, false, false,
               false, true);
    }
  }
  state.koList.clear();
...
inline void KOProc2(State &state) {
  ...
  if (state.koList.empty()) {
    ...
    for (const AreaRef &ref : state.koList) {
      const Card *card = state.checkGetCard(ref);
      if (card != nullptr) {
        PullTrigger(state, TriggerType::Ko, ref.card, CardRef(card->koCauseRef),
                    1);
        if (card->koEnemyAttackDamage) {
          PullTrigger(state, TriggerType::KoEnemyAttackDamage, ref.card,
                      CardRef(card->koCauseRef), 1);
        }
        if (card->koEnemyExAttackDamage) {
          PullTrigger(state, TriggerType::KoEnemyExAttackDamage, ref.card,
                      CardRef(card->koCauseRef), 1);
        }
        if (card->koEnemyAttackDamageActive) {
          PullTrigger(state, TriggerType::KoEnemyAttackDamageActive, ref.card,
                      CardRef(card->koCauseRef), 1);
        }
  ...
  for (int i : state.basicPlayerOrder()) {
    ...
    if (ps.tool.size() >= 2) {
      ...
      for (const RefPosition &rp : inPlayPokemon) {
        ...
        const int remain = state.remainingToolCapacity(card);
        if (remain < 0) {
          state.stateChanged = true;
    ...
    for (int i = ps.energy.size() - 1; i >= 0; i--) {
      ...
      if (master.onlyTeamRocket) {
        ...
        if (!state.getCard(rp.ref).getMaster().teamRocket) {
          state.stateChanged = true;
          MoveCard(state, state.activePlayerIndex(), AreaType::Energy, i,
                   AreaType::Trash);
        }
      }
    }
...
inline void BenchCheck(State &state) {
  for (int i : {1, 0}) {
    const int playerIndex = (i ^ state.lastStadiumPlayer);
...
inline void SetAndActivateSkill(State &state, const Skill &skill,
                                const Card &card, CardRef ref,
                                int usePlayerIndex) {
  const ActivateAbilityInfo info{skill.skillId, AreaRef(ref, card.moveCounter),
                                 usePlayerIndex};
