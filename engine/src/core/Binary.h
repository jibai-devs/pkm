...
static inline const std::string base64String =
...
public:
  ...
  void set(const void *pFirst, const void *pLast) {
    set(pFirst, static_cast<const uint8_t *>(pLast) -
                    static_cast<const uint8_t *>(pFirst));
  }

  template <typename T> void set(const std::vector<T> &list) {
    set(static_cast<int>(list.size()));
    set(list.data(), sizeof(T) * list.size());
  }

  void set(int num) { set(&num, sizeof(int)); }
...
public:
  ...
  void set(void *p, size_t numBytes) {
    ...
    std::copy(start, start + numBytes, static_cast<uint8_t *>(p));
    pos += numBytes;
  }

  void set(void *pFirst, void *pLast) {
    set(pFirst, static_cast<uint8_t *>(pLast) - static_cast<uint8_t *>(pFirst));
  }

  template <typename T> void set(std::vector<T> &list) {
    list.resize(readInt());
    set(list.data(), sizeof(T) * list.size());
  }

  int readInt() {
    int v = buf[pos] + (buf[pos + 1] << 8) + (buf[pos + 2] << 16) +
            (buf[pos + 3] << 24);
    pos += 4;
    return v;
  }
  ...
  void fromBase64() {
    base64Dest.clear();
    int last = static_cast<int>(base64.size());
    ...
    int padding = 0;
    int inputCount = static_cast<int>(base64Dest.size());
    ...
    for (int i = 0; i < inputCount; i += 4) {
      int val = (toValue(base64Dest[i]) << 18) +
                ...
                (toValue(base64Dest[i + 2]) << 6) + toValue(base64Dest[i + 3]);
