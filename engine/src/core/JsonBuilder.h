...
public:
  ...
  void appendDoubleQuote(const char *str) {
    append(u8'"');
    buf += reinterpret_cast<const char8_t *>(str);
    append(u8'"');
  }
  ...
  template <int Size> void appendStr(const char (&ss)[Size]) {
    std::u8string_view v{reinterpret_cast<const char8_t *>(ss), Size - 1};
    buf += v;
  }
  ...
  void append(char8_t c) { buf += c; }

  void append(char c) { buf += static_cast<char8_t>(c); }
  ...
  void append(int num) {
    ...
    if (num <= 9) {
      append(static_cast<char8_t>(u8'0' + num));
      return;
    }
    ...
    int digit = 0;
    while (n > 0) {
      c[digit] = static_cast<char8_t>(u8'0' + n % 10);
      digit++;
      n = n / 10;
    }
  ...
  void append(signed char num) { append(static_cast<int>(num)); }

  void append(unsigned char num) { append(static_cast<int>(num)); }

  void append(size_t num) { append(static_cast<int>(num)); }

  void appendNull() { appendStr("null"); }
  ...
  template <int Size> void appendKey(const char (&key)[Size]) {
    append(u8'"');
    appendStr(key);
    append(u8'"');
    append(u8':');
  ...
  template <int Size> void appendCommaKey(const char (&key)[Size]) {
    append(u8',');
    appendKey(key);
  }

  template <int Size> void appendKeyValue(const char (&key)[Size], int val) {
    appendKey(key);
    append(val);
  }
  template <int Size> void appendKeyValue(const char (&key)[Size], bool val) {
    appendKey(key);
    append(val);
  }
  template <int Size>
  ...
  // 空文字列ならnull
  template <int Size>
  void appendCommaKeyValueOrNull(const char (&key)[Size],
