...
// 最大容量を指定するリスト
template <typename SizeT, typename T, SizeT Capacity> class FixedListBase {
public:
  FixedListBase() noexcept { cnt = 0; }
  ...
  auto end() { return buffer.begin() + cnt; }
  auto end() const { return buffer.begin() + cnt; }
...
protected:
  ...
  void checkIndex(int index) const {
    ...
    if (index >= cnt) {
      Exception("invalid index " + std::to_string(index) +
                " >= " + std::to_string(cnt));
    }
  }
...
constexpr std::ranges::iota_view<int, int> range(size_t count) {
  return std::ranges::iota_view<int, int>{0, static_cast<int>(count)};
}

template <typename T>
constexpr std::ranges::iota_view<int, int> range(const std::vector<T> &list) {
  return std::ranges::iota_view<int, int>{0, static_cast<int>(list.size())};
}

template <typename T, size_t Capacity>
constexpr std::ranges::iota_view<int, int>
range(const std::array<T, Capacity> &ar) {
  return std::ranges::iota_view<int, int>{0, static_cast<int>(ar.size())};
}

template <typename SizeT, typename T, SizeT Capacity>
constexpr std::ranges::iota_view<int, int>
range(const FixedListBase<SizeT, T, Capacity> &list) {
