...
class Chain {
public:
  explicit Chain(CardMaster &card) : card(card) {}

  CardMaster &getCard() { return card; }
  ...
  Chain &pokemon(PokemonType pokemonType, EvolutionType evolutionType,
                 EnergyType energyType, int hp, int retreatCost) {
  ...
  // 特性追加
  Chain &ability(int skillId, const char8_t *name, const char8_t *text,
                 AreaType areaType = AreaType::All,
                 AreaType areaType2 = AreaType::All) {
  ...
  Chain &abilityBattleField(int skillId, const char8_t *name,
                            const char8_t *text) {
  ...
  Chain &abilityBenchToActive(int skillId, const char8_t *name,
                              const char8_t *text) {
  ...
  Chain &abilityActiveToBench(int skillId, const char8_t *name,
                              const char8_t *text) {
  ...
  Chain &activateSkillOnceTurn(int skillId, const char8_t *name,
                               const char8_t *text) {
    abilityBattleField(skillId, name, text);
    sk->mainAbility = true;
    sk->onceTurn = true;
  ...
  Chain &activateSkillOnceTurnActive(int skillId, const char8_t *name,
                                     const char8_t *text) {
    abilityActive(skillId, name, text);
    sk->mainAbility = true;
    sk->onceTurn = true;
  ...
  Chain &activateSkillOnceTurnBench(int skillId, const char8_t *name,
                                    const char8_t *text) {
    abilityBench(skillId, name, text);
    sk->mainAbility = true;
    sk->onceTurn = true;
  ...
  Chain &activateSkillFirstTurn(int skillId, const char8_t *name,
                                const char8_t *text) {
  ...
  // ワザ追加
  Chain &attack(int attackId, const char8_t *name, const char8_t *text,
                const char *damage,
                std::initializer_list<EnergyType> energies) {
  ...
  Chain &attack(int attackId, int damage,
                std::initializer_list<EnergyType> energies) {
  ...
  Chain &preEffectAttackDamageChange(int damageChange) {
    return preEffect(EffectType::AttackDamageChange, TargetPlayer::None)
        .eVal(damageChange);
  }

  Chain &preEffectAttackDamageChangeCoin(int coinCount, int damageChange) {
    return preEffect(EffectType::AttackDamageChangeCoin, TargetPlayer::None)
        .eVal(coinCount, damageChange);
  }

  Chain &preEffectUnitedWings() {
    return preEffect(EffectType::AttackDamageChangeTargetCount,
                     TargetPlayer::Me)
        .eVal(20)
        .targetTrash()
        .targetNameCondition(TargetType::HasAttackName, u8"だんけつのつばさ");
  }
  ...
  Chain &preEffectRound(int damage) {
    return preEffect(EffectType::AttackDamageChangeTargetCount,
        ...
        .eVal(damage)
        .targetPokemon()
        .targetNameCondition(TargetType::HasAttackName, u8"りんしょう");
  }
  ...
  Chain &postEffectDamagePokemon(int damage, int pokemonCount = 1) {
    ...
    effect(EffectType::AttackDamage, TargetPlayer::Enemy)
        .eVal(damage)
        .targetPokemon()
        .multiSelect(pokemonCount);
    return *this;
  }
  ...
  Chain &postEffectBreakIfNotPostEffectActivated() {
    return postEffect(EffectType::BreakIfNotPostEffectActivated,
                      TargetPlayer::None);
  }

  Chain &postEffectTrashEnergyMe(int count) {
    return postEffect(EffectType::ToTrash, TargetPlayer::Me)
  ...
  Chain &postEffectParalyzeIfCoinHead() {
    return setPostEffect().effectBreakIfCoinTail().effect(EffectType::Paralyze,
                                                          TargetPlayer::Enemy);
  }

  // コインを1回投げオモテなら、次の相手の番、このポケモンはワザのダメージや効果を受けない
  Chain &postEffectNoDamageAndEffectIfCoinHead() {
    return setPostEffect().effectBreakIfCoinTail().effectMe(
        EffectType::NoDamageAndEffectAttackNextEnemyTurn);
  }
  ...
  Chain &effectInit(EffectType type, TargetPlayer targetPlayer) {
    ...
    *ef = {};
    ...
    } else if (type == EffectType::ToHand || type == EffectType::PrizeToHand ||
               type == EffectType::ToHandReverse ||
               type == EffectType::ToHandWithAttach) {
      ef->selectContext = SelectContext::ToHand;
    } else if (type == EffectType::ToTrash ||
               type == EffectType::TransformTrash) {
      ef->selectContext = SelectContext::Discard;
    } else if (type == EffectType::ToDeckAndShuffle ||
               type == EffectType::ToDeck ||
               type == EffectType::ToDeckReverse ||
               type == EffectType::LookToDeckReverse ||
               type == EffectType::ToDeckReverseAndShuffle ||
               type == EffectType::ToDeckWithAttach ||
               type == EffectType::SwitchDeck) {
      ef->selectContext = SelectContext::ToDeck;
    } else if (type == EffectType::ToDeckBottom ||
               type == EffectType::ToDeckBottomReverse ||
               type == EffectType::ToDeckBottomClose) {
    ...
    } else if (type == EffectType::DamageCounter ||
               type == EffectType::DamageCounterRemoved) {
    ...
    } else if (type == EffectType::RemoveDamageCounter ||
               type == EffectType::RemoveDamageCounterAll) {
    ...
    } else if (type == EffectType::SelectEvolvesFrom ||
               type == EffectType::EvolvesFromEach) {
      ef->selectContext = SelectContext::EvolvesFrom;
    } else if (type == EffectType::EvolvesToEach ||
               type == EffectType::SelectEvolvesTo) {
      ef->selectContext = SelectContext::EvolvesTo;
    } else if (type == EffectType::AttachFromEach ||
               type == EffectType::SelectAttachFrom ||
               type == EffectType::AttachSelectedCard ||
               type == EffectType::SwitchSelectedCard ||
               type == EffectType::EnergySwitchEach) {
      ef->selectContext = SelectContext::AttachFrom;
    } else if (type == EffectType::AttachToEach ||
               type == EffectType::SelectAttachTo ||
               type == EffectType::AttachEnergyMe) {
  ...
  Chain &effect(EffectType type, TargetPlayer targetPlayer) {
    assert(sk != nullptr ||
           attackEffectType != 0); // ワザの最初でこの関数を呼んではいけない
  ...
  Chain &condition(ConditionType type, int value = 0,
                   ComparatorType comparatorType = ComparatorType::Equal) {
    if (isSkill()) {
  ...
  Chain &conditionCountTargetMeOrEnemy(int count0, int count1,
  ...
  Chain &conditionGreaterEqual(int count,
                               TargetPlayer player = TargetPlayer::Me) {
  ...
  Chain &existStadium() { return exist(AreaType::Stadium, TargetPlayer::Both); }

  Chain &existMyDeck() { return exist(AreaType::Deck); }

  Chain &existEnemyDeck() { return exist(AreaType::Deck, TargetPlayer::Enemy); }

  Chain &existMyTrash() { return exist(AreaType::Trash); }

  Chain &existMyHand() { return exist(AreaType::Hand); }

  Chain &existMyBench() { return exist(AreaType::Bench); }
  ...
  Chain &conditionAttachEnergyMe(EnergyType type) {
    return exist(AreaType::Me)
        .targetCondition(TargetType::IsAttachedEnergyType, (int)type);
  }

  // このポケモンに指定タイプのエネルギーが2個以上ついているなら
  Chain &conditionAttachEnergy2Me(EnergyType type) {
    return exist(AreaType::Me)
        .targetCondition(TargetType::IsAttachedEnergy2Type, (int)type);
  }
  ...
  Chain &conditionMyTurn() { return condition(ConditionType::MyTurn); }
  ...
  Chain &conditionPrizeCountGreater() {
    return condition(ConditionType::CompareCountTargetMeEnemy, 0,
  ...
  Chain &targetHand() { return target(AreaType::Hand); }

  Chain &targetTrash() { return target(AreaType::Trash); }

  Chain &targetDeck() { return target(AreaType::Deck); }
  ...
  Chain &targetActive() { return target(AreaType::Active); }

  Chain &targetBench() { return target(AreaType::Bench); }

  Chain &targetPrize() { return target(AreaType::Prize); }

  Chain &targetAttachedEnergy() { return target(AreaType::Energy); }

  Chain &targetAttachedTool() { return target(AreaType::Tool); }

  Chain &targetLooking() { return target(AreaType::Looking); }

  Chain &
  targetCondition(TargetType type, int val = 0,
                  ComparatorType comparatorType = ComparatorType::Equal) {
  ...
  Chain &
  targetNameCondition(TargetType type, const char8_t *name,
                      ComparatorType comparatorType = ComparatorType::Equal) {
    TargetCondition condition = {};
    condition.targetType = type;
    condition.comparatorType = comparatorType;
  ...
  Chain &
  targetCondition2(TargetType type, int val, int val2,
                   ComparatorType comparatorType = ComparatorType::Equal) {
    TargetCondition condition = {};
    condition.targetType = type;
    condition.comparatorType = comparatorType;
  ...
  Chain &targetEnergyCard() { return targetCondition(TargetType::EnergyCard); }

  Chain &targetItem() { return targetCondition(TargetType::Item); }

  Chain &targetToolCard() { return targetCondition(TargetType::Tool); }

  Chain &targetSupporter() { return targetCondition(TargetType::Supporter); }

  Chain &targetStadiumCard() { return targetCondition(TargetType::Stadium); }

  Chain &targetTrainer() { return targetCondition(TargetType::Trainer); }
  ...
  Chain &targetDamaged() {
    return targetCondition(TargetType::DamageCounter, 0,
                           ComparatorType::Greater);
  }
  ...
  Chain &targetNotMyCardId() {
    return targetCondition(TargetType::CardId, card.cardId,
  ...
  // 1枚選ぶ
  Chain &singleSelect() { return multiSelect(1); }
  ...
  // 順番を選ぶ
  Chain &allSelect() { return select(EffectSelectType::CardCount, 99); }
  ...
  Chain &noEffect() { return effect(EffectType::NoEffect, TargetPlayer::Both); }
  ...
  Chain &effectLookAndToHandReverseRestDeckBottom(int lookCount) {
    ...
    effect(EffectType::ToHandReverse, TargetPlayer::Me)
        .targetLooking()
        .singleSelect();
    int effectIndex = currentEffectIndex();
  ...
  Chain &effectLookAndToHandRestDeckAndShuffle(int lookCount,
                                               int toHandCount = 1) {
    existMyDeck();
    effect(EffectType::LookDeck, TargetPlayer::Me).eVal(lookCount);
    effect(EffectType::ToHand, TargetPlayer::Me)
        .targetLooking()
        .maxSelect(toHandCount);
    int effectIndex = currentEffectIndex();
  ...
  Chain &effectLookAndToHandReverseRestDeckAndShuffle(int lookCount,
                                                      int toHandCount = 1) {
    existMyDeck();
    effect(EffectType::LookDeck, TargetPlayer::Me).eVal(lookCount);
    effect(EffectType::ToHandReverse, TargetPlayer::Me)
  ...
  Chain &effectLookDeckBottomAndToHandRestDeckAndShuffle(int lookCount,
                                                         int toHandCount = 1) {
    existMyDeck();
  ...
  Chain &effectLookAndToBenchRestDeckAndShuffle(int lookCount,
                                                TargetPlayer player) {
    ...
    effect(EffectType::ToBench, player)
        .targetLooking()
        .targetPokemonCard()
        .maxSelect(lookCount);
    int effectIndex = currentEffectIndex();
  ...
  Chain &effectDeckToHandCardId(int cardId) {
    ...
    effect(EffectType::ToHand, TargetPlayer::Me)
        .targetDeck()
        .targetCondition(TargetType::CardId, cardId);
    return maxSelect(1);
  }
  ...
  Chain &effectDeckToBenchAndShuffle(int count = 1) {
    notFullBench();
    existMyDeck();
    effect(EffectType::ToBench, TargetPlayer::Me)
        .targetDeck()
        .targetPokemonCard();
    maxSelect(count);
    int effectIndex = currentEffectIndex();
  ...
  Chain &effectTrashStadium() {
    ...
    return effect(EffectType::ToTrash, TargetPlayer::Both)
  ...
  Chain &effectSelectAttachBasicEnergyTrash(int count = 1) {
    ...
    int effectIndex = currentEffectIndex();
    effect(EffectType::SelectAttachTo, TargetPlayer::Me)
        .targetTrash()
        .targetBasicEnergy();
    if (count == 1) {
      singleSelect();
  ...
  Chain &effectSelectAttachEnergyDeck(int count = 1) {
    existMyDeck();
    return effect(EffectType::SelectAttachTo, TargetPlayer::Me)
        .targetDeck()
        .targetEnergyCard()
        .maxSelect(count);
  }
  ...
  Chain &effectAttachFromEach() {
    return effect(EffectType::AttachFromEach, TargetPlayer::Me)
        .eachSelectedList()
        .singleSelect();
  }
  ...
  Chain &effectHandAttachEnergyMe(int count = 1) {
    exist(AreaType::Hand);
    int effectIndex = currentEffectIndex();
    effect(EffectType::AttachEnergyMe, TargetPlayer::Me)
        .targetHand()
        .targetEnergyCard();
    if (count == 1) {
      singleSelect();
  ...
  Chain &effectSwitch(TargetPlayer player) {
    ...
    int effectIndex = currentEffectIndex();
    effect(EffectType::Switch, player).targetBench().singleSelect();
    return setSecondTarget(effectIndex);
  }
  ...
  Chain &effectTrashEnergy(int count, TargetPlayer player) {
    ...
    int effectIndex = currentEffectIndex();
    effect(EffectType::ToTrash, player)
        .targetAttachedEnergy()
        .selectEnergy(count);
    return setSecondTarget(effectIndex);
  }
  ...
  Chain &effectEnergySwitchEach(TargetPlayer player) {
    return effect(EffectType::EnergySwitchEach, player)
        .eachSelectedList()
        .targetCondition(TargetType::NotContextCardAttachedPokemon)
        .singleSelect();
  }
  ...
  Chain &effectBasicEnergySwitch(TargetPlayer player) {
    ...
    effect(EffectType::SelectSwitchEnergyCard, player)
        .targetAttachedEnergy()
        .targetBasicEnergy()
        .maxSelectEnergy(1);
  ...
  Chain &effectHealSingle(int heal) {
    ...
    return effect(EffectType::Heal, TargetPlayer::Me)
        .eVal(heal)
        .targetPokemon()
        .targetDamaged()
        .singleSelect();
  ...
  Chain &effectSelectAttachFrom(int count) {
    existPokemon();
    int effectIndex = currentEffectIndex();
    effect(EffectType::SelectAttachFrom, TargetPlayer::Me)
        .targetPokemon()
        .maxSelect(count);
    return setSecondTarget(effectIndex);
  }
  ...
  Chain &effectDeckSelectBasicEnergyNotSameType(int count,
                                                SelectContext context) {
    effect(EffectType::SelectCard, TargetPlayer::Me)
        .targetDeck()
        .singleSelect();
  ...
  Chain &effectAttachToEach() {
    return effect(EffectType::AttachToEach, TargetPlayer::Me)
        .eachSelectedList();
  }

  Chain &effectEvolvesToEach() {
    return effect(EffectType::EvolvesToEach, TargetPlayer::Me)
        .targetCondition(TargetType::CanEvolveContextCard)
        .eachSelectedList()
        .maxSelect(1);
  }

  Chain &effectEvolvesFromEach() {
    return effect(EffectType::EvolvesFromEach, TargetPlayer::Me)
        .targetCondition(TargetType::CanEvolvesToContextCard)
        .eachSelectedList()
        .singleSelect();
  }
  ...
  Chain &effectBreakIfCoinTailMulti(int count) {
    return effect(EffectType::BreakIfCoinTailMulti, TargetPlayer::None)
        .eVal(count);
  }
...
private:
  ...
  [[nodiscard]] bool isSkill() const { return sk != nullptr; }

  [[nodiscard]] int currentEffectIndex() {
    return static_cast<int>(currentEffects().size()) - 1;
  }
...
inline Chain &CreateCard(int cardId, const char8_t *name, CardType cardType,
                         int no) {
