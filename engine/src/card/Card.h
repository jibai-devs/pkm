...
#include "card/Skill.h"

struct CardRef {
  unsigned char cardIndex;

  CardRef() = default;
  explicit CardRef(int cardIndex)
      : cardIndex(static_cast<unsigned char>(cardIndex)) {}

  [[nodiscard]] bool isNull() const { return cardIndex == 0; }
};
...
inline bool operator!=(AreaRef left, AreaRef right) { return !(left == right); }
...
struct CardMaster {
  ...
  bool
      cannotToHandOrDeckInTrash{}; // トラッシュにあるかぎり、手札に加えられず、山札にもどせない
  bool canPlayFirstTurn{}; // 最初のターンに使用可能
  bool transformOnly{};    // 変身でしか場に出せない
  bool
      canTrash{}; // 自分の番の中でなら、場に出ているこのカードをトラッシュできる
  bool toBench{}; // 【たね】ポケモンとして、場に出る
  bool
      toBattleFieldOnlySetup{}; // 対戦準備でのみ【たね】ポケモンとして、場に出る
  bool
      toActiveOnlySetup{}; // 対戦準備でポケモンをバトル場に出すとき、このカードが手札にあるなら、ウラにしてバトル場に出してよい
  bool noPrize{}; // このカードが【きぜつ】しても、相手はサイドをとれない
  bool
      onlyTeamRocket{}; // 「ロケット団のポケモン」にしかつけられず、「ロケット団のポケモン」以外についているなら、トラッシュする
  ...
  // 化石や人形は含まない
  [[nodiscard]] bool isPokemonCard() const {
    return cardType == CardType::Pokemon;
  }

  // 対戦準備で出せるカード
  [[nodiscard]] bool canSetup() const {
    return (cardType == CardType::Pokemon &&
            evolutionType == EvolutionType::Basic) ||
           toBattleFieldOnlySetup;
  }

  [[nodiscard]] bool canSetupActive() const {
    return canSetup() || toActiveOnlySetup;
  }

  // 化石や人形を含む
  [[nodiscard]] bool isPokemon() const {
    return pokemonType != PokemonType::NotPokemon;
  }

  // ルールを持つポケモン
  [[nodiscard]] bool isRulePokemon() const {
    return pokemonType == PokemonType::Ex || pokemonType == PokemonType::MegaEx;
  }

  [[nodiscard]] bool isEx() const {
    return pokemonType == PokemonType::Ex || pokemonType == PokemonType::MegaEx;
  }

  // ルールを持たないポケモン
  // 化石や人形を含む
  [[nodiscard]] bool isNotRulePokemon() const {
    return !isRulePokemon() && pokemonType != PokemonType::NotPokemon;
  }

  // ルールを持たないポケモン
  // 化石や人形は含まない
  [[nodiscard]] bool isNotRulePokemonCard() const {
    return pokemonType == PokemonType::Normal;
  }

  [[nodiscard]] bool hasAttack(const Attack *attack) const {
  ...
  [[nodiscard]] FixedList<const Skill *, 2> getSkills() const {
...
union CardNextTurnState {
  ...
  struct {
    short
        cannotUseAttackId; // ワザの選択自体ができない。ワザが使われることがあっても失敗する
...
struct Card {
  ...
  union {
    unsigned short
        nextEnemyTurnEndStateBattleField; // ベンチにもどっても消えない
    struct {
  ...
  union {
    unsigned
        nextEnemyTurnEndState; // 次の相手の番の終わりまで(自分にかける効果)
    struct {
  ...
  [[nodiscard]] const CardMaster &getMaster() const {
    return CardTable.at(cardId);
  }

  [[nodiscard]] ByteFixedList<EnergyType, 2> getEnergyType() const {
