...
#include "mechanics/PullTrigger.h"
#include "mechanics/SetProperty.h"

inline void RefreshEffect(State &state, int depth = 0);
inline CardRef MoveCardAttach(State &state, int playerIndex, AreaType fromArea,
                              int fromIndex, AreaType toArea, bool ko);
...
inline std::pair<CardRef, int> MoveCardFromLast(FromList &from, ToList &to) {
  int index = static_cast<int>(from.size()) - 1;
...
inline CardRef MoveCard(State &state, int playerIndex, AreaType fromArea,
                        int fromIndex, AreaType toArea, int openType = 0,
                        bool noLog = false, bool withAttach = false,
                        bool mustMove = false, bool ko = false) {
  ...
  if (fromArea == AreaType::Trash) {
    if (toArea == AreaType::Hand || toArea == AreaType::Deck ||
        toArea == AreaType::DeckBottom) {
  ...
  if (!mustMove) {
    ...
    if (toArea == AreaType::Bench) {
      ...
      if (fromArea == AreaType::Hand ||
          (fromArea == AreaType::Looking && card.preArea == AreaType::Hand)) {
  ...
  if (toArea == AreaType::Stadium) {
    if (!state.stadium.empty()) {
      if (fromArea != AreaType::Stadium) {
        ...
        MoveCard(state, card.playerIndex, AreaType::Stadium, 0,
                 AreaType::Trash);
      }
    }
  }
  ...
  if ((fromArea == AreaType::Active && toArea != AreaType::Bench) ||
    ...
    for (int i = ps.preEvolution.size() - 1; i >= 0; i--) {
      ...
      if (state.getCard(ref).attachMoveCounter == moveCounter) {
        if (withAttach) {
          MoveCardAttach(state, playerIndex, AreaType::PreEvolution, i, toArea,
                         ko);
        } else {
          if (toArea == AreaType::Hand) {
            MoveCardAttach(state, playerIndex, AreaType::PreEvolution, i,
                           AreaType::Hand, ko);
          } else {
            MoveCardAttach(state, playerIndex, AreaType::PreEvolution, i,
                           AreaType::Trash, ko);
    ...
    for (int i = ps.energy.size() - 1; i >= 0; i--) {
      ...
      if (state.getCard(ref).attachMoveCounter == moveCounter) {
        if (withAttach) {
          ...
          MoveCardAttach(state, playerIndex, AreaType::Energy, i,
                         AreaType::Trash, ko);
        }
      }
    }
    for (int i = ps.tool.size() - 1; i >= 0; i--) {
      ...
      if (state.getCard(ref).attachMoveCounter == moveCounter) {
        if (withAttach) {
          ...
          MoveCardAttach(state, playerIndex, AreaType::Tool, i, AreaType::Trash,
                         ko);
        }
      }
    }
  ...
  if (toArea == AreaType::Bench) {
    ...
    if (fromArea == AreaType::Deck) {
      if (state.onEffect()) {
        ...
        if (type == CardType::Pokemon || type == CardType::Item ||
...
inline CardRef MoveCardAttach(State &state, int playerIndex, AreaType fromArea,
                              int fromIndex, AreaType toArea, bool ko) {
  return MoveCard(state, playerIndex, fromArea, fromIndex, toArea, 0, false,
                  false, true, ko);
}

inline CardRef MoveCardBase(State &state, int playerIndex, AreaType fromArea,
                            int fromIndex, AreaType toArea) {
  return MoveCard(state, playerIndex, fromArea, fromIndex, toArea);
}

inline void MoveRefCard(State &state, CardRef ref, const Card &card,
                        AreaType toArea, int openType = 0, bool noLog = false,
                        bool withAttach = false) {
  int index = state.currentAreaIndex(ref, card);
  MoveCard(state, card.playerIndex, card.area, index, toArea, openType, noLog,
           withAttach);
}
...
inline void SwitchPokemon(State &state, int playerIndex, int benchIndex) {
  ...
  CardRef activeRef;
  if (ps.active.empty()) {
    activeRef = MoveCard(state, playerIndex, AreaType::Bench, benchIndex,
                         AreaType::Active);
    RefreshEffect(state);
  } else {
...
inline void EvolveProc(State &state, AreaType area, int index,
                       AreaType inPlayArea, int inPlayIndex, int playerIndex) {
  PlayerState &ps = state.players[playerIndex];
  CardRef &inPlayRef =
      (inPlayArea == AreaType::Active ? ps.active.at(0)
                                      : ps.bench.at(inPlayIndex));
  Card &preEvolveCard = state.getCard(inPlayRef);
  if (preEvolveCard.area != AreaType::Active &&
      preEvolveCard.area != AreaType::Bench) {
    return;
  }
...
inline void DevolveProc(State &state, CardRef ref, Card &card,
                        AreaType toArea) {
  ...
  for (int i = ps.preEvolution.size() - 1; i >= 0; i--) {
    ...
    if (preCard.attachMoveCounter == card.moveCounter) {
      ...
      CardRef &currentRef =
          (area == AreaType::Active ? ps.active.at(0) : ps.bench.at(areaIndex));
...
inline void AttachProc(State &state, AreaType area, int index, int playerIndex,
                       CardRef targetRef, bool isEffect) {
  CardRef ref = state.getCardRef(area, index, playerIndex);
  Card &card = state.getCard(ref);
  const CardMaster &master = card.getMaster();

  const Card &targetCard = state.getCard(targetRef);
  if (targetCard.area != AreaType::Active &&
      targetCard.area != AreaType::Bench) {
    return;
  }
  ...
  if (IsEnergy(master.cardType) &&
      !state.canAttachEnergy(card, master, targetRef)) {
    return;
  }
  ...
  if (area == AreaType::Hand) {
    if (IsEnergy(master.cardType)) {
      ...
      for (int i = (int)state.delayTriggerStack.size() - 1; i >= 0; i--) {
        ...
        if (ta.trigger.type == TriggerType::EnergyAttachFromHand &&
            ta.trigger.subject.card == targetRef) {
    ...
    if (master.play != nullptr) {
      ...
      ta.activateInfo = ActivateAbilityInfo(
          master.play->skillId, state.makeAreaRef(ref), card.playerIndex);
      state.temporaryTriggerStack.push_back(ta);
    }
  }
}

inline void SwitchEnergyProc(State &state, CardRef energyRef,
                             CardRef pokemonRef) {
  ...
  LogMoveAttached(state, pokemonCard.playerIndex, energyRef, rp.ref,
                  pokemonRef);
