...
struct Effect {
  ...
  bool
      multiplyEffectValueCoinHeadCount{}; // コインでオモテが出たの数をvaluesにかける
  ...
  bool
      selectEnemyEnergyCount{}; // 相手のポケモン全員についているエネルギーの数ぶんまで選択できる
  bool skipNoTarget{}; // 対象がいなければ選択スキップ
  bool open{};         // 公開する
  bool
      setTargetSwitchBench{}; // 入れ替えでベンチに行ったポケモンをtargetListに入れる
  ...
  ComparatorType comparatorType{};
  signed char
      failSkip{}; // 設定されていたら、conditionが満たされなかったときに、この数だけ飛ばす
  int skillId{};
  ...
  bool isNotOpenSelectNoCondition() const {
    if (isNotOpenSelect() && !target.conditions.empty()) {
      return true;
    }
    return false;
  }
...
struct Skill {
  ...
  signed char
      secondEffectStartIndex{}; // 2つから効果を選べる場合の、2つ目の開始インデックス
  signed char
      secondEffectStartIndexEnemy{}; // 相手が2つから効果を選べる場合の、2つ目の開始インデックス
  ...
  bool hasTrigger() const { return !triggers.empty(); }
...
struct Attack {
  ...
  union {
    ...
    struct {
      ...
      bool asEnemyDeckTop10Attack
          : 1; // 相手の山札を上から10枚オモテにする。のぞむなら、その中にあるポケモンが持つワザを1つ選び、このワザとして使う
      bool noTargetEffect
          : 1; // このワザのダメージは、対象のポケモンにかかっている効果を計算しない
      ...
      bool cannotUseSameNameAttackPreMyTurn
          : 1; // 前の自分の番に、自分のポケモンが同名のワザを使っていたなら、このワザは使えない
      bool canOnlyUseEnemyPrizeOne
          : 1; // このワザは、相手のサイドの残り枚数が1枚のときにしか使えない
      bool deckTopAttack
          : 1; // 自分の山札を上から1枚トラッシュし、そのカードがポケモン（「ルールを持つポケモン」をのぞく）なら、そのポケモンが持っているワザを1つ選び、このワザとして使う
      bool deckTopSupporter
          : 1; // 自分の山札を上から1枚トラッシュし、そのカードがサポートなら、その効果を、このワザの効果として使う
      bool noEnergyIfSpecialCondition
          : 1; // このワザは、このポケモンが特殊状態なら、このワザを使うためのエネルギーがこのポケモンについていなくても、使える。
      bool darkness1IfDamaged
          : 1; // このワザは、このポケモンにダメカンがのっているなら、【悪】エネルギー1個で使える
      bool prizePlus1
          : 1; // このワザのダメージで、相手のポケモンが【きぜつ】したなら、サイドを1枚多くとる
      bool koNoDamageAndEffectAttackNextEnemyTurn
          : 1; // このワザのダメージで、相手のポケモンが【きぜつ】したなら、次の相手の番、このポケモンはワザのダメージや効果を受けない
  ...
  std::vector<Effect>
      preEffects; // ワザの成否やダメージに影響がある効果や「ダメージを与える前に」と書かれた効果
