...
inline void ApiBattleFinish(ApiData *data) { delete data; }
...
inline SerialData ApiGetBattleData(ApiData *data) {
  ...
  return {data->jsonBuilder.buf.c_str(), b.base64.data(),
          static_cast<int>(b.base64.size()), state.selectPlayer};
}
...
inline void ApiVisualizeData(JsonBuilder &j,
                             const std::vector<std::u8string> &visData) {
  j.clear();
  j.append('[');
...
inline SearchInfo ApiSearchBegin(ApiData *data,
                                 const SearchStartConfig &config) {
  return data->search.start(config, data->state);
}

inline SearchInfo ApiSearchStep(ApiData *data, long long searchId, int *select,
                                int selectCount) {
  data->selected.clear();
  for (int i : range(selectCount)) {
...
inline void ApiSearchEnd(ApiData *data) { data->search.clear(); }
...
inline void ApiAllCard(JsonBuilder &j) {
  ...
  std::sort(cards.begin(), cards.end(),
            [](const CardMaster *left, const CardMaster *right) {
  ...
  for (int i : range(cards)) {
    ...
    j.appendCommaKeyValue("cardType", static_cast<int>(card.cardType));
    j.appendCommaKeyValue("pokemonType", static_cast<int>(card.pokemonType));
    j.appendCommaKeyValue("evolutionType",
                          static_cast<int>(card.evolutionType));
    ...
    j.appendCommaKeyValue("stage1",
                          card.evolutionType == EvolutionType::Stage1);
    j.appendCommaKeyValue("stage2",
...
inline void ApiAllAttack(JsonBuilder &j) {
  ...
  std::sort(attacks.begin(), attacks.end(),
            [](const Attack *left, const Attack *right) {
