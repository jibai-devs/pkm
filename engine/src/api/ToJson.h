...
inline void CardJson(const State &state, JsonBuilder &j, CardRef ref,
                     bool canReverse, bool addName = false) {
...
template <typename T>
inline void CardListJson(const State &state, JsonBuilder &j, const T &list,
                         bool canReverse, bool addName) {
  j.append('[');
...
inline void PokemonJson(const State &state, JsonBuilder &j, CardRef ref,
                        bool addName) {
  const Card &card = state.getCard(ref);
  if (card.reverse && !addName) {
    ...
    const CardMaster &master = card.getMaster();
    const int playerIndex = card.playerIndex;
    j.append('{');
...
template <typename T>
inline void PokemonListJson(const State &state, JsonBuilder &j, const T &list,
                            bool addName) {
  j.append('[');
  for (int i = 0; i < list.size(); i++) {
    j.comma(i);
...
inline void PlayerJson(const State &state, JsonBuilder &j, int pi,
                       int myPlayerIndex) {
  const bool addName = (myPlayerIndex == 2);
...
inline void Current(const State &state, JsonBuilder &j, int playerIndex,
                    bool web) {
  const bool addName = (playerIndex == 2);
  j.append('{');
  ...
  if (state.looking.empty() || (state.lookingPlayer != playerIndex &&
                                state.lookingPlayer != 2 && playerIndex != 2)) {
    if (!state.looking.empty() && state.lookingPlayer >= 3 &&
        state.lookingPlayer == playerIndex + 3) {
      j.append('[');
...
inline void SelectJson(const State &state, JsonBuilder &j, bool web) {
  ...
  if (web) {
    j.appendKey("type");
    j.appendDoubleQuote(SelectTypeStr[static_cast<int>(state.selectType)]);

    j.appendCommaKey("context");
    j.appendDoubleQuote(
        SelectContextStr[static_cast<int>(state.selectContext)]);
  } else {
    j.appendKeyValue("type",
                     std::max(0, static_cast<int>(state.selectType) - 1));
    j.appendCommaKeyValue(
        "context", std::max(0, static_cast<int>(state.selectContext) - 1));
  }
  ...
  j.appendCommaKey("deck");
  if (state.selectDeck) {
    CardListJson(state, j, state.players[state.selectPlayer].deck, false,
                 false);
  } else {
    j.appendNull();
  }
...
inline void LogsJson(const State &state, JsonBuilder &j, int playerIndex,
                     int startLogIndex, bool web) {
  auto &logList = state.logs;
  const int logSize = static_cast<int>(logList.size()) - startLogIndex;
  int counter = 0;
  j.append('[');
...
inline void ToJsonWeb(const State &state, JsonBuilder &j, int playerIndex,
                      int startLogIndex, int responseIndex) {
  j.clear();
  j.append('{');
  ...
  if (state.selectType == SelectType::None ||
      state.selectPlayer != playerIndex) {
    j.appendNull();
  } else {
...
inline void ToJsonSearch(State *state, JsonBuilder &j, int searchId,
                         int error) {
  j.clear();
...
inline void ToJsonVis(const State &state, JsonBuilder &j, int startLogIndex,
