...
inline void PokemonCheckup(State &state) {
  ...
  for (int i : range(2)) {
    ...
    if (ps.badStatus == BadStatusType::Asleep ||
        ps.badStatus == BadStatusType::Paralyzed || ps.isPoisoned() ||
        ps.isBurned()) {
    ...
    if (!state.noToolEffect) {
      for (int i = ps.tool.size() - 1; i >= 0; i--) {
        ...
        if (master.trashMyTurnEnd) {
          MoveCard(state, state.activePlayerIndex(), AreaType::Tool, i,
                   AreaType::Trash);
        }
      }
    }
...
inline void TurnEnd(State &state) {
  ...
  for (int i = static_cast<int>(state.delayTriggerStack.size()) - 1; i >= 0;
       i--) {
    ...
    if (effectCard.playerIndex != playerIndex) {
      ...
      if (ta.trigger.type == TriggerType::TurnEnd &&
          ta.trigger.subject.card == state.playerCardRef(playerIndex)) {
        state.temporaryTriggerStack.push_back(ta);
        state.delayTriggerStack.erase(state.delayTriggerStack.begin() + i);
      }
    }
  }
  PullTrigger(state, TriggerType::TurnEnd,
              state.playerCardRef(state.activePlayerIndex()));
...
inline void AfterPlay(State &state) {
  ...
  while (state.playing.size() > 0) {
    MoveCard(state, state.activePlayerIndex(), AreaType::Playing, 0,
             AreaType::Trash, false, true);
  }
}
...
inline void SelectedPlay(State &state, int handIndex) {
  ...
  if (master.cardType == CardType::Pokemon) {
    MoveCard(state, playerIndex, AreaType::Hand, handIndex, AreaType::Bench, 0,
             true);
  } else if (master.cardType == CardType::Stadium) {
    state.stadiumPlayed = true;
    MoveCard(state, playerIndex, AreaType::Hand, handIndex, AreaType::Stadium,
             0, true);
  } else {
    ...
    if (master.toBench) {
      MoveCard(state, playerIndex, AreaType::Hand, handIndex, AreaType::Bench,
               0, true);
    } else {
      MoveCard(state, playerIndex, AreaType::Hand, handIndex, AreaType::Playing,
               0, true);
...
inline void SelectedAttach(State &state, int handIndex, AreaType area,
                           int index) {
  int playerIndex = state.activePlayerIndex();
...
inline void SelectedEvolve(State &state, int handIndex, AreaType area,
                           int index) {
  EvolveProc(state, AreaType::Hand, handIndex, area, index,
...
inline void SelectedRetreat2(State &state) {
  ...
  if (cost > 0) {
    SelectTrashSinglePokemonEnergy(
        state, playerIndex, state.players[playerIndex].getActive(), cost);
  }
}
...
inline void SelectedRetreat(State &state) {
  ...
  int playerIndex = state.activePlayerIndex();
  PullTrigger(state, TriggerType::PreRetreat,
              state.players[playerIndex].getActive());
...
inline bool CanUseAttack(const State &state, CardRef ref, const Card &card,
                         const AttackEnergy &ae) {
        ...
        if (attacker.doubleAttack) {
          if (ps.badStatus != BadStatusType::Asleep &&
            ...
            if (attacker.getMaster().hasAttack(&state.getAttack())) {
              ...
              state.setSelect(SelectType::Attack, SelectContext::Attack,
                              playerIndex, 0, 1);
...
inline void AttackDamage2(State &state, int damage) {
  ...
  AddDamage(state, targetRef, target, damage, true, attackerRef, false,
            &attack);
...
inline void AttackDamage(State &state) {
  ...
  if (!calcDamage) {
    for (const Effect &effect : attack.preEffects) {
      if (effect.effectType == EffectType::AttackDamageChange ||
          ...
          effect.effectType ==
              EffectType::AttackDamageChangeTypeEnergyCountCoin ||
          ...
          effect.effectType ==
              EffectType::AttackDamageChangeTargetCountEnemyCoin ||
          ...
          effect.effectType ==
              EffectType::AttackDamageChangeSpecialConditionCount ||
          effect.effectType ==
              EffectType::AttackDamageChangeTakeAttackDamagePreTurn ||
          effect.effectType ==
              EffectType::AttackDamageChangePreTurnTakePrizeCount) {
        calcDamage = true;
        break;
      }
    }
  ...
  if (calcDamage) {
    ...
    int damage = CalcDamage(state, baseDamage, targetRef, target, attackerRef,
...
inline void AttackEffects(State &state, bool preAttack) {
  ...
  const std::vector<Effect> &effects =
      (preAttack ? attack.preEffects : attack.postEffects);
  if (effects.size() > 0) {
    state.pushFunction(AttackEffect);
    state.setLastFunctionCallCount(static_cast<int>(effects.size()));
  }
}
...
inline void SpecialAttackProc(State &state) {
  ...
  if (srcAttack.deckTopAttack) {
    ...
    CardRef ref = MoveCard(state, playerIndex, AreaType::Deck,
                           ps.deck.size() - 1, AreaType::Trash);
    const Card &card = state.getCard(ref);
    const CardMaster &master = card.getMaster();
  ...
  } else if (srcAttack.asActiveEnemyPokemonAttack ||
             srcAttack.asActiveEnemyPokemonAttackIfCoinHead ||
             srcAttack.asActiveEnemyTerastalPokemonAttack) {
    const Card &card = state.getCard(state.getAttacker());
    const CardMaster &enemy =
        state.getCard(state.players[1 - card.playerIndex].getActive())
            .getMaster();
    ...
    for (const Attack *a : enemy.attacks) {
      if (srcAttack.asActiveEnemyPokemonAttack &&
          a->asActiveEnemyPokemonAttack) {
        continue;
      }
      AddOptionAttack(state, a->attackId, srcAttack.attackId);
    }
    ...
    for (int i : range(10)) {
      ...
      CardRef ref = MoveCard(state, playerIndex, AreaType::Deck,
                             ps.deck.size() - 1, AreaType::Looking);
    ...
    if (master.cardType == CardType::Supporter) {
      state.pushFunction(AfterAttack);
      ActivateAbilityInfo info(master.play->skillId,
                               state.makeAreaRef(state.attacker), playerIndex);
...
inline void SelectedAttack(State &state, int attackId, int srcAttackId,
                           int benchIndex) {
  PlayerState &ps = state.players[state.activePlayerIndex()];
...
inline void SelectedMain(State &state) {
  ...
  if (selected.type == SelectOptionType::End) {
    ...
    case SelectOptionType::Attach:
      SelectedAttach(state, selected.param1,
                     static_cast<AreaType>(selected.param2), selected.param3);
      break;
    case SelectOptionType::Evolve:
      SelectedEvolve(state, selected.param1,
                     static_cast<AreaType>(selected.param2), selected.param3);
      break;
    case SelectOptionType::Ability:
      SelectedAbility(state, static_cast<AreaType>(selected.param0),
                      selected.param1);
      break;
    case SelectOptionType::Discard:
      SelectedDiscard(state, static_cast<AreaType>(selected.param0),
                      selected.param1);
      break;
    case SelectOptionType::Retreat:
      ...
      break;
    default:
      Exception("unexpected option type " +
                std::to_string(static_cast<int>(selected.type)));
      break;
    }
  }
}

inline bool CanRetreat(State &state, int playerIndex,
                       const std::vector<EnergyType> &energyList) {
  if (state.retreated) {
    return false;
  }
...
inline void MainSelect(State &state) {
  ...
  for (int i : range(ps.hand)) {
    ...
    if (master.cardType == CardType::Pokemon) {
      ...
      if (ps.thisTurn.cannotPlaySpecialEnergy &&
      ...
      if (master.cardType == CardType::Supporter) {
        ...
        if (master.cardId == ANGE_FLOETTE) {
          ...
          if (state.getCard(state.stadium[0]).getMaster().name !=
              u8"プリズムタワー") {
            continue;
          }
        } else {
        ...
        if (ae.insufficientEnergy <= 0) {
          if (CanUseAttack(state, active, card, ae)) {
            if (ae.srcAttackId == 0 || ae.srcAttackId == ae.attack->attackId) {
              if (!state.satisfyAttackCondition(card, *ae.attack,
                                                ae.srcAttackId)) {
                continue;
              }
            } else {
              if (!state.satisfyAttackCondition(
                      card, AttackTable.at(ae.srcAttackId), ae.srcAttackId)) {
                continue;
              }
            }
            if (state.turn >= 2 || ae.attack->canUseFirst ||
                card.canAttackFirst) {
              AddOptionAttack(state, ae.attack->attackId, ae.srcAttackId);
            }
          }
    ...
    for (int i : range(ps.bench)) {
      ...
      for (const Attack *attack : master.attacks) {
        if (attack->canUseBench) {
          ...
          if (EnoughEnergy(state, attack->energies, energyList2, card,
                           *attack)) {
