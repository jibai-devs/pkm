...
inline int InsufficientEnergyCount(const State &state,
                                   const AttackEnergies &required,
                                   const std::vector<EnergyType> &energies,
                                   const Card &card, const Attack &attack,
                                   bool returnExtra = false) {
  EnergyTable requiredTable = {};
  bool fixEnergy = false;
  if (attack.noEnergyIfSpecialCondition &&
      state.players[card.playerIndex].isSpecialCondition()) {
    fixEnergy = true;
  } else if ((card.attackEnergyColoressOne || card.attackEnergyPsychicOne) &&
             card.getMaster().hasAttack(&attack)) {
    fixEnergy = true;
  ...
  if (!fixEnergy) {
    ...
    int change =
        card.attackCostChangeColorless + card.thisTurn.attackCostChange;
    if (change < 0) {
  ...
  if (returnExtra) {
    ...
    int needCount = requiredTable.sum - allCount -
                    std::min(colorlessCount, requiredTable.colorless);
    return std::max(needCount, 0);
  }
}

inline bool EnoughEnergy(const State &state, const AttackEnergies &required,
                         const std::vector<EnergyType> &energies,
                         const Card &card, const Attack &attack) {
  return InsufficientEnergyCount(state, required, energies, card, attack) <= 0;
}

inline void SetAttackEnergy(const State &state, const Card &card,
                            const std::vector<EnergyType> &energyList,
                            bool extract, bool ownOnly = false) {
  ...
  for (const Attack *attack : master.attacks) {
    int count = InsufficientEnergyCount(state, attack->energies, energyList,
                                        card, *attack);
    if (count <= 0) {
      if (extract) {
        if (attack->asActiveEnemyPokemonAttack) {
          const CardMaster &enemy =
              state.getCard(state.players[1 - playerIndex].getActive())
                  .getMaster();
  ...
  if (!ownOnly) {
    if (card.canUsePreEvolutionAttack) {
      for (CardRef preEvolutionRef :
           state.players.at(playerIndex).preEvolution) {
        ...
        if (card.moveCounter == preEvolutionCard.attachMoveCounter) {
          for (const Attack *attack : preEvolutionCard.getMaster().attacks) {
            int count = InsufficientEnergyCount(state, attack->energies,
                                                energyList, card, *attack);
            result.push_back({attack, count});
          }
        }
      }
    ...
    if (card.technicalMachine) {
      for (CardRef ref : state.getAttachedToolRef(card)) {
        ...
        if (toolMaster.name == u8"コアメモリ" &&
            master.name != u8"メガジガルデex") {
          continue;
        }
