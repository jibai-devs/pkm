...
#include "mechanics/ActivateInfo.h"
...
struct CardPosition {
  ...
  CardPosition(AreaType area, int areaIndex, int playerIndex)
      : area(area), areaIndex(areaIndex), playerIndex(playerIndex) {}
};
...
struct AttachedToolEnergy {
  ...
  const std::u8string &name() const { return card->getMaster().name; }
};
...
struct SelectOption {
  ...
  CardPosition getCardPosition() const {
    return {static_cast<AreaType>(param0), param1, param2};
  }
};
...
struct State {
  ...
  signed char
      lookingPlayer{}; // 見ているプレイヤーのインデックス。2なら双方、3と4は裏向きで見ている
  ...
  SelectContext
      selectContext{}; // 厳密に設定されているわけではない。期待と違うcontextな可能性もあるので必ず確かめる
  ...
  std::array<Card, MAX_CARD_COUNT> allCard;
  ...
  void clear() {
    ...
    firstPlayer = 0;
    lookingPlayer = 0;
    lookingReverse = false;
    isBreak = false;
    effectLoopStop = false;
    changed = false;
    stateChanged = false;
    updateOrder = false;
    turnState = 0;
    ...
    energyCost = 0;
    remainEnergyCost = 0;
    selectedEnergyCardCount = 0;
    removedDamageCounter = 0;
    selectCounts = {};
    selectingEnergyPokemonRef = {};
    effectState = {};
    triggerInfo = {};
    effectJump = 0;
    attachActive = false;
    currentAttackId = 0;
    srcAttackId = 0;
    attackDamageChange = 0;
    lastAttackDamage = 0;
    attacker = {};
    postAttackEffect = false;
    postEffectActivate = false;
    failAttack = false;
    secondAttack = false;
    moveCounter = 0;
    currentSkillOrder = 0;
  ...
  bool isFinish() const { return gameResult != GameResult::None; }
  ...
  bool isPlayerTurn() const { return phase == GamePhase::Main; }
  ...
  TurnHistory &thisTurnHistory() { return turnHistories[0]; }

  bool isAddLog() const { return game->isAddLog(); }
  Log &addLog(LogType type) { return logs.emplace_back(type); }

  int nextLogStart() {
    int playerIndex = selectPlayer;
    int logStart = logIndex[playerIndex];
    logIndex.at(playerIndex) = static_cast<int>(logs.size());
    return logStart;
  }

  void setSelect(SelectType selectType, SelectContext selectContext,
                 int selectPlayer, int selectMin = 1, int selectMax = 1) {
    if (selectContext == SelectContext::None) {
      Exception("no context");
    }
    if (SelectContext::Main < selectContext &&
        selectContext < SelectContext::DiscardEnergyCard) {
  ...
  void setSelectMinMaxAny() {
    selectMin = 0;
    selectMax = static_cast<int>(options.size());
  }
  ...
  SelectOption &addOption(SelectOptionType type) {
    if (selectType == SelectType::Card) {
      ...
      if (type != SelectOptionType::EnergyCard &&
          type != SelectOptionType::ToolCard) {
  ...
  SelectOption firstSelected() const { return selectedOption(0); }
  ...
  int selectedNumber() const { return firstSelected().param0; }
  ...
  CardRef getCardRef(AreaType area, int areaIndex, int playerIndex) const {
    ...
    default:
      Exception("getCardRef unexpected area " +
  ...
  CardRef removeCardRef(AreaType area, int areaIndex, int playerIndex) {
    ...
    default:
      Exception("removeCardRef unexpected area " +
                std::to_string(static_cast<int>(area)));
      return {};
    }
  }
  ...
  void pushCardRef(AreaType area, int playerIndex, CardRef ref) {
    ...
    default:
      Exception("pushCardRef unexpected area " +
                std::to_string(static_cast<int>(area)));
    }
  }
  ...
  CardRef getAttachCardRef(const SelectOption &option, int playerIndex) const {
    AreaType area = static_cast<AreaType>(option.param0);
    return getCardRef(area, option.param1, playerIndex);
  }

  CardRef getAttachPokemonCardRef(const SelectOption &option,
                                  int playerIndex) const {
    AreaType area = static_cast<AreaType>(option.param2);
  ...
  CardId getCardId(CardRef ref) const { return getCard(ref).cardId; }
  ...
  void cardMoved(CardRef ref, AreaType newArea, bool reverse = false) {
    ...
    if (preArea == AreaType::Active) {
      ...
      for (int i = static_cast<int>(delayTriggerStack.size()) - 1; i >= 0;
           i--) {
    ...
    if (newArea != AreaType::Active) {
      if (newArea == AreaType::Bench && preArea == AreaType::Active) {
        bool cannotAttackLessEqualEnergy2ThisTurn =
            card.thisTurn.cannotAttackLessEqualEnergy2;
        bool cannotAttackLessEqualEnergy2NextTurn =
            card.nextTurn.cannotAttackLessEqualEnergy2;
        card.clearNextTurnState();
        card.thisTurn.cannotAttackLessEqualEnergy2 =
            cannotAttackLessEqualEnergy2ThisTurn;
        card.nextTurn.cannotAttackLessEqualEnergy2 =
            cannotAttackLessEqualEnergy2NextTurn;
      } else {
        card.clearNextTurnState();
      }
  ...
  int currentAreaIndex(int playerIndex, AreaType area, CardRef ref) const {
    ...
    default:
      Exception("currentAreaIndex unexpected area " +
                std::to_string(static_cast<int>(area)));
    }
    return -1;
  }
  ...
  RefPosition
  attachedEnergyPositionFromOption(const SelectOption &option) const {
    assert(option.type == SelectOptionType::Energy ||
  ...
  AttachedCardList getAttachedList(const PlayerState &ps,
                                   const CardList &list) {
  ...
  int getHp(const Card &card) const { return getMaxHp(card) - card.damage; }
  ...
  EnergyType getEnergyType(const Card &card) const {
    ...
    return static_cast<EnergyType>(type);
  }
  ...
  bool isKo(const Card &card) const { return getMaxHp(card) <= card.damage; }

  bool cannotToHand(CardRef ref) const {
    const Card &card = getCard(ref);
    if (card.area == AreaType::Energy || card.area == AreaType::Tool ||
        card.area == AreaType::PreEvolution) {
  ...
  int energyCount(int playerIndex, CardRef pokemonRef) const {
    ...
    return static_cast<int>(game->energyList.size());
  }

  int typeEnergyCount(int playerIndex, CardRef pokemonRef,
                      EnergyType type) const {
    int count = 0;
    getEnergies(playerIndex, pokemonRef, game->energyList);
  ...
  void getEnergies(int playerIndex, CardRef pokemonRef,
                   std::vector<EnergyType> &output) const {
    output.clear();
    const PlayerState &ps = players.at(playerIndex);
  ...
  int targetListEnergyCount() {
    ...
    for (const AreaRef &ref : targetList) {
      ...
      if (card != nullptr) {
        ...
        count += static_cast<int>(game->energyList.size());
      }
    }
    return count;
  }
  ...
  EnergyInfo getEnergyInfo(const Card &energyCard, CardRef pokemonRef) const {
    ...
    if (master.cardType == CardType::SpecialEnergy) {
      ...
      if (master.cardId == NEO_UPPER_ENERGY) {
        ...
        const CardMaster &master = pokemonCard.getMaster();
        if (master.evolutionType == EvolutionType::Stage1 ||
  ...
  int retreatCost(const Card &card) const {
    ...
    int cost = master.retreatCost + card.retreatCostChange +
               card.thisTurn.retreatCostChange;
  ...
  int getPrizeCount(const Card &card) const {
    ...
    if (card.koEnemyTerastalAttackDamage && card.area == AreaType::Active) {
      count += players[1 - card.playerIndex]
                   .takePrizeCountChangeTerastalAttackKoActive;
    }
    if (card.koEnemyNAttackDamage && card.area == AreaType::Active) {
      count +=
          players[1 - card.playerIndex].takePrizeCountChangeNAttackKoActive;
    }
  ...
  // ワザの特殊使用条件を満たしているか
  bool satisfyAttackCondition(const Card &card, const Attack &attack,
                              int srcAttackId) const {
    if (card.thisTurn.cannotUseAttackId == attack.attackId ||
        card.thisTurn.cannotUseAttackId2 == attack.attackId ||
        card.cannotUseAttackIdNonActive == attack.attackId) {
  ...
  int activePlayerIndex() const { return ((turn + 1) ^ firstPlayer) & 1; }
  ...
  // カード種類によらない共通処理
  bool canPlay(const PlayerState &ps, const Card &card,
               const CardMaster &master) const {
  ...
  bool canEvolveEffect(const State &state, const Card &handCard,
                       const CardMaster &handMaster, CardRef pokemonRef) const {
    ...
    if (pokemonCard.rainbowDna) {
      if (state.turn > 2 && !pokemonCard.appear &&
          handCard.area == AreaType::Hand && handMaster.isEx() &&
  ...
  bool canEvolve(const State &state, const Card &handCard,
                 const CardMaster &handMaster, CardRef pokemonRef,
                 bool cannotAppearThisTurn = true) const {
    if (handMaster.cardType != CardType::Pokemon) {
      return false;
    }
    const Card &pokemonCard = getCard(pokemonRef);
    ...
    if (cannotAppearThisTurn && pokemonCard.appear) {
      if (cannotEvolveAppearTurn &&
          (!pokemonCard.canEvolveGrassAppearTurn ||
           !ContainsEnergyType(handMaster.energyType, EnergyType::Grass))) {
        return false;
      }
    }
  ...
  bool canEvolve2(const Card &handCard, const CardMaster &handMaster,
                  CardRef pokemonRef) const {
    if (handMaster.cardType != CardType::Pokemon) {
      return false;
    }
    const Card &pokemonCard = getCard(pokemonRef);
  ...
  bool canAttachEnergy(const Card &energyCard, const CardMaster &energyMaster,
                       CardRef pokemonRef) const {
    const Card &pokemonCard = getCard(pokemonRef);
  ...
  FixedList<AttachedToolEnergy, 4>
  getAttachedTools(const Card &pokemonCard) const {
    ...
    if (onAttackEffect()) {
      ...
      if (attacker.playerIndex != card.playerIndex) {
        ...
        if (card.noEffectEnemyAttack ||
            card.noDamageAndEffectEnemyAttackNextEnemyTurn) {
          return true;
        }
      ...
      if (effectCard.playerIndex != card.playerIndex) {
        ...
        if (card.noEnemyAbility) {
          if (effectCard.area == AreaType::Active ||
              effectCard.area == AreaType::Bench) {
            return true;
          }
        }
      }
  ...
  bool isPreventDamageCounter(const Card &targetPokemon) {
    ...
    if (targetPokemon.noDamageCounterEnemyAttackAbility) {
      if (effectCard.getMaster().isPokemon() &&
          targetPokemon.playerIndex != effectCard.playerIndex) {
        return true;
      }
    }
    return false;
  ...
  bool noSpecialCondition(const Card &card) const {
    return card.noSpecialCondition ||
           card.getMaster().pokemonType == PokemonType::PokemonItem;
  }

  bool afterConfuse(const Card &card) {
    return players.at(card.playerIndex).badStatus == BadStatusType::Confused &&
           card.notRecoverConfuseEvolve;
  }
  ...
  bool onSkill() const { return effectState.ability.skillId > 0; }

  bool onEffect() const { return effectState.onEffect; }

  bool onAttack() const { return currentAttackId > 0; }
  ...
  auto getValuesByEffect(const Effect &effect) const {
    ...
    if (effect.multiplyEffectValuePreTargetCount) {
      for (int &v : values) {
        v *= static_cast<int>(preTargetList.size());
      }
    }
  ...
  // Instant Effect処理中のみ
  AreaRef getEffectCard() const { return effectState.ability.effectCard; }

  // Instant Effect処理中のみ
  int getEffectValue() const { return getValueByEffect(getEffect()); }

  // Instant Effect処理中のみ
  int getSecondEffectValue() const { return getValuesByEffect(getEffect())[1]; }
  ...
  // Instant Effect処理中のみ
  int effectPlayerIndex() const { return effectState.ability.usePlayerIndex; }
  ...
  // Attack中のみ
  const Attack &getAttack() const { return AttackTable.at(currentAttackId); }

  // Attack中のみ
  CardRef getAttacker() const { return attacker; }

  void breakEffect() { effectJump = 99; }

  static FixedList<int, 2> effectTargetPlayerContinual(const Effect &effect,
                                                       int effectPlayer) {
  ...
  GameFunction &lastFunction() { return functionStack.back(); }
  ...
  int calledCount() const { return functionStack.back().calledCount; }
  ...
  void callFunction() {
    ...
    int index = static_cast<int>(functionStack.size()) - 1;
    ...
    case ArgType::I:
      ...
      break;
    case ArgType::B:
      ((GameFunctionB)fp)(*this, static_cast<bool>(gf.arg0));
      break;
    case ArgType::II:
