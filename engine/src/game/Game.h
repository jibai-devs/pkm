...
struct Log {
  ...
  void add(int val) { param.push_back(val); }

  void add(AreaType area) { param.push_back(static_cast<int>(area)); }
};
...
struct Game {
  ...
  std::function<void(void)>
      pushResponseFunc; // クライアントへ送るレスポンスを追加
  ...
  bool isAddLog() const { return !selecting && config.recordLog; }
