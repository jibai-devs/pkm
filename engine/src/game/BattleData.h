...
public:
  ...
  void setState(const GameConfig &config) {
    ...
    int cardIndex = 0;
    state.game = &game;
    state.moveCounter = INITIAL_MOVE_COUNTER;
    state.firstPlayer = NO_FIRST_PLAYER;
  ...
  void init(const GameConfig &config, bool checkDeck = true) {
    ...
    for (int i : range(2)) {
      ...
      game.remainingTime[i] = config.timeLimit * MS_PER_SECOND;
    }
  }

  void start() { state.pushFunction(SetupGame); }
  ...
  bool next() {
    ...
    if (!state.game->selecting) {
      ...
      if (state.game->actionCount >= MAX_ACTION_COUNT) {
        state.setResult(DRAW_PLAYER_INDEX, FinishReason::Other);
        isContinue = false;
      }
    }

    if (!isContinue) {
      int reason = static_cast<int>(state.finishReason);
      LogResult(state, static_cast<int>(state.gameResult) - 1, reason);
    }
    return isContinue;
  }
};
...
inline void InitializeBattleFunction() {
  ...
  for (int i : range(FunctionTable)) {
    FunctionIndexTable[reinterpret_cast<long long>(FunctionTable[i])] = i;
  }
}
