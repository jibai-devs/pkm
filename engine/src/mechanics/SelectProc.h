...
#include "card/CardMove.h"

inline void SelectPokemonEnergyLoop(State &state, int playerIndex);
inline void SelectPrize(State &state, int playerIndex, int count,
                        int coinPrizePlus);
...
inline void SelectCoinSingle(State &state, int playerIndex) {
  if (state.game->config.manualCoin) {
    ...
    auto r = (state.game->config.deviceRand ? std::random_device()()
                                            : state.game->rng());
    bool head = (r % 2 == 0);
    if (head) {
      state.coinHeadCount++;
...
inline void SelectedCoinUntilTail(State &state, int playerIndex) {
  ...
  if (head) {
    state.coinHeadCount++;
    constexpr int maxCoinTossCount = 10'000'000;
    if (state.coinHeadCount >= maxCoinTossCount) {
      return;
    }
    state.setSelect(SelectType::YesNo, SelectContext::CoinHead, playerIndex);
    AddOptionYesAndNo(state);
...
inline void SelectedSwitchPokemon(State &state) {
  const CardPosition pos = state.firstSelected().getCardPosition();
  state.clearSelect();

  SwitchPokemon(state, pos.playerIndex, pos.areaIndex);
}

inline void SelectSwitchPokemon(State &state, int switchPlayer,
                                int selectPlayer) {
...
inline void SelectBenchMaxTrash(State &state, int playerIndex) {
  ...
  const int trashCount = -state.remainingBench(playerIndex);
  state.setSelect(SelectType::Card, SelectContext::Discard, playerIndex,
...
inline void SelectToolMaxTrash(State &state, int playerIndex, int cardIndex) {
  ...
  if (remain < 0) {
    ...
    state.setSelect(SelectType::AttachedCard, SelectContext::DiscardToolCard,
                    playerIndex, trashCount, trashCount);
    for (const AttachedCard &ac : list) {
      if (ref == ac.targetRef) {
        AddOptionToolCard(state, pos.area, pos.areaIndex, playerIndex,
                          ac.attachedIndex);
      }
    }
...
inline void AfterEnergyDiscard(State &state, CardRef energyRef,
                               const Card &energyCard, int attachMoveCounter) {
  ...
  if (master.cardId == BOOMERANG_ENERGY || master.cardId == NITRO_FIRE_ENERGY) {
    if (state.onAttack() && state.onEffect()) {
      ...
      if (attachMoveCounter == attacker.moveCounter &&
          attachMoveCounter == effectCard.moveCounter) {
        if (attacker.area == AreaType::Active ||
            attacker.area == AreaType::Bench) {
          if (master.cardId == NITRO_FIRE_ENERGY) {
            if (!ContainsEnergyType(state.getEnergyType(attacker),
                                    EnergyType::Fire)) {
              return;
            }
          }
          ...
          ta.activateInfo = ActivateAbilityInfo(master.ability->skillId,
                                                state.makeAreaRef(energyRef),
                                                energyCard.playerIndex);
...
inline void SelectedPokemonEnergyLoop(State &state, int playerIndex) {
  const SelectContext context = state.selectContext;
  if (!state.selected.empty()) {
    const SelectOption o = state.firstSelected();
    state.clearSelect();
    const RefPosition rp = state.attachedEnergyPositionFromOption(o);
    ...
    if (!state.onEffect() || !state.isPreventEffect(energyCard)) {
      if (state.selectContext == SelectContext::DiscardEnergy) {
        const int moveCounter = energyCard.attachMoveCounter;
        MoveCard(state, energyCard.playerIndex, AreaType::Energy, rp.index,
                 AreaType::Trash);
...
inline void SelectPokemonEnergyLoop(State &state, int playerIndex) {
  ...
  state.setSelect(SelectType::Energy, state.selectContext, playerIndex,
                  selectMin, 1);
  for (const AreaRef &tref : state.targetList) {
    if (!Contains(state.selectedList, tref.card)) {
      ...
      const int index = state.energyIndex(tref.card, rp.ref);
      if (state.onEffect() && state.isPreventEffect(energyCard)) {
        continue;
      }
      AddOptionEnergy(state, rp.area, rp.index, energyCard.playerIndex, index,
                      ei.count);
    }
  }
...
inline void SelectPokemonEnergy(State &state, int playerIndex,
                                const AttachedCardList &energies, int cost,
...
inline void SelectTrashSinglePokemonEnergy(State &state, int playerIndex,
                                           CardRef targetRef, int cost) {
  AttachedCardList energies;
  const int moveCounter = state.getCard(targetRef).moveCounter;
  for (CardRef ref : state.players[playerIndex].energy) {
    if (state.getCard(ref).attachMoveCounter == moveCounter) {
      energies.push_back({ref, targetRef, energies.size()});
    }
  }
  SelectPokemonEnergy(state, playerIndex, energies, cost,
                      SelectContext::DiscardEnergy);
}
...
inline void SelectedPrizeToHand(State &state, int cardIndex) {
  const CardRef ref(cardIndex);
  const CardPosition pos = state.currentCardPosition(ref);
...
inline void SelectedLuckyBonus(State &state, int cardIndex) {
  const bool isYes = state.selectedYes();
  state.clearSelect();
  const CardRef ref(cardIndex);
  const Card &card = state.getCard(ref);
  const int index =
      state.currentAreaIndex(card.playerIndex, AreaType::Temporary, ref);
  if (isYes) {
    MoveCard(state, card.playerIndex, AreaType::Temporary, index,
             AreaType::Bench, 0);
...
inline void SelectedPrizeTarget(State &state) {
  ...
  for (int i = static_cast<int>(state.targetList.size()) - 1; i >= 0; i--) {
    ...
    if (card.reverse && ability != nullptr && ability->luckyBonus &&
        state.remainingBench(card.playerIndex) > 0 && state.isPlayerTurn() &&
        state.activePlayerIndex() == card.playerIndex) {
      const int index = state.currentAreaIndex(ref.card, card);
...
inline void SelectPrize2(State &state, int playerIndex, int count) {
  const PlayerState &ps = state.players.at(playerIndex);
  const int selectCount = std::min(count, ps.prize.size());
  state.setSelect(SelectType::Card, SelectContext::ToHand, playerIndex,
