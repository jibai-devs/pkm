...
#include "mechanics/SatisfyCondition.h"

inline bool IsTriggerTarget(const State &state, CardRef ref,
                            const Target &target, CardRef effectCardRef,
...
inline void TriggerList(State &state, TriggerType triggerType, CardRef subject,
                        CardRef object, CardRef effectCard, int depth) {
  ...
  for (const Trigger &trigger : skill.triggers) {
    ...
    if (!skill.effects.empty()) {
      bool satisfy = true;
      const TriggerInfo preInfo = state.triggerInfo;
      state.triggerInfo = ta.trigger;
      const Effect &effect = skill.effects[0];
      if (effect.isCondition) {
        const ConditionType type = effect.conditionType;
        if (type == ConditionType::MyTurn || type == ConditionType::Turn ||
            type == ConditionType::KoPreEnemyTurn ||
            type == ConditionType::KoAttackDamagePreEnemyTurn) {
          if (!SatisfyCondition(state, skill.effects, 0, effectCard,
                                card.playerIndex)) {
            satisfy = false;
          }
        }
    ...
    if (skill.onceTurn) {
      const AreaRef ref = state.makeAreaRef(effectCard);
      bool found = false;
      for (TriggeredAbility &ta : state.triggerStack) {
        if (ta.activateInfo.skillId == skill.skillId &&
            ta.activateInfo.effectCard == ref) {
          found = true;
          break;
        }
      }
    ...
    ta.activateInfo = ActivateAbilityInfo(
        skill.skillId, state.makeAreaRef(effectCard), card.playerIndex);
...
inline void PullTrigger(State &state, TriggerType triggerType, CardRef subject,
                        CardRef object = {}, int depth = 0) {
  for (int i = 0; i < 2; i++) {
