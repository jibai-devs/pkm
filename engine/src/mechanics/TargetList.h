...
inline bool IsTargetSingle(const State &state, CardRef ref, const Card &card,
                           const CardMaster &master,
                           const TargetCondition &target,
                           const AreaRef &effectCard) {
  ...
  case TargetType::EnergyType:
    return BoolCompare(MatchEnergyType(state.getEnergyType(card),
                                       static_cast<EnergyType>(target.val)),
                       target.comparatorType);
  case TargetType::EnergyType2:
    return BoolCompare(
        MatchEnergyType(state.getEnergyType(card),
                        static_cast<EnergyType>(target.val)) ||
            MatchEnergyType(state.getEnergyType(card),
                            static_cast<EnergyType>(target.val2)),
        target.comparatorType);
  case TargetType::Resistance:
    return BoolCompare(
        MatchEnergyType(master.resistance, static_cast<EnergyType>(target.val)),
        target.comparatorType);
  ...
  case TargetType::BasicPokemon:
    if (card.area == AreaType::Active || card.area == AreaType::Bench) {
      return BoolCompare(master.evolutionType == EvolutionType::Basic,
                         target.comparatorType);
    } else {
      return BoolCompare(master.evolutionType == EvolutionType::Basic &&
                             master.cardType == CardType::Pokemon,
                         target.comparatorType);
    }
  case TargetType::EvolvedPokemon:
    return BoolCompare(master.evolutionType == EvolutionType::Stage1 ||
  ...
  case TargetType::Stage2:
    return BoolCompare(master.evolutionType == EvolutionType::Stage2,
                       target.comparatorType);
  case TargetType::BasicEnergy:
    return BoolCompare(master.cardType == CardType::BasicEnergy,
                       target.comparatorType);
  case TargetType::SpecialEnergy:
    return BoolCompare(master.cardType == CardType::SpecialEnergy,
                       target.comparatorType);
  case TargetType::EnergyCard:
    return BoolCompare(IsEnergy(master.cardType), target.comparatorType);
  case TargetType::Item:
    return BoolCompare(master.cardType == CardType::Item,
                       target.comparatorType);
  case TargetType::Tool:
    return BoolCompare(master.cardType == CardType::Tool,
                       target.comparatorType);
  case TargetType::Supporter:
    return BoolCompare(master.cardType == CardType::Supporter,
                       target.comparatorType);
  case TargetType::Stadium:
    return BoolCompare(master.cardType == CardType::Stadium,
                       target.comparatorType);
  ...
  case TargetType::PokemonOrBasicEnergy:
    return BoolCompare(master.cardType == CardType::Pokemon ||
  ...
  case TargetType::NotRulePokemonCardOrBasicEnergy:
    return BoolCompare(master.isNotRulePokemonCard() ||
                           master.cardType == CardType::BasicEnergy,
                       target.comparatorType);
  case TargetType::EnergyTypePokemonOrStadium: {
    bool match = false;
    if (master.cardType == CardType::Pokemon) {
      match = MatchEnergyType(state.getEnergyType(card),
                              static_cast<EnergyType>(target.val));
  ...
  case TargetType::EthanPokemonOrBasicFireEnergy:
    return BoolCompare(master.ethan || master.cardId == FIRE_ENERGY,
                       target.comparatorType);
  case TargetType::HasAbility:
    return BoolCompare(state.getAbility(card, master) != nullptr,
                       target.comparatorType);
  case TargetType::HasAbilityName: {
    const Skill *ability = state.getAbility(card, master);
    return BoolCompare(ability != nullptr && ability->name == target.name,
                       target.comparatorType);
  }
  ...
  case TargetType::MegaEx:
    return BoolCompare(master.pokemonType == PokemonType::MegaEx,
                       target.comparatorType);
  ...
  case TargetType::SilcoonOrCascoon:
    return BoolCompare(master.name == u8"カラサリス" ||
                           master.name == u8"マユルド",
                       target.comparatorType);
  case TargetType::KoffingOrWeezing:
    return BoolCompare(master.name.find(u8"ドガース") != std::u8string::npos ||
                           master.name.find(u8"マタドガス") !=
                               std::u8string::npos,
                       target.comparatorType);
  case TargetType::HonedgeOrDoubladeOrAegislash:
    return BoolCompare(master.name == u8"ヒトツキ" ||
                           master.name == u8"ニダンギル" ||
                           master.name == u8"ギルガルド",
                       target.comparatorType);
  case TargetType::Name:
    return BoolCompare(master.name == target.name, target.comparatorType);
  case TargetType::NameContains:
    return BoolCompare(master.name.find(target.name) != std::u8string::npos,
                       target.comparatorType);
  case TargetType::CanEvolve:
    ...
    for (RefPosition rp :
         state.players.at(card.playerIndex).getInPlayPokemon()) {
  ...
  case TargetType::CanEvolveMe: {
    return BoolCompare(
        state.canEvolveEffect(state, card, master, effectCard.card),
        target.comparatorType);
  }
  case TargetType::CanEvolveContextCard: {
    ...
    return BoolCompare(
        state.canEvolveEffect(state, card, master, state.contextCard),
        target.comparatorType);
  }
  case TargetType::CanEvolvesToContextCard: {
    ...
    return BoolCompare(
        state.canEvolveEffect(state, contextCard, contextCard.getMaster(), ref),
        target.comparatorType);
  }
  ...
  case TargetType::AttachedMe:
    return BoolCompare(card.attachMoveCounter ==
                           state.getCard(effectCard.card).moveCounter,
                       target.comparatorType);
  ...
  case TargetType::AttachedTriggerSubject: {
    ...
    return BoolCompare(card.attachMoveCounter == state.getCard(r).moveCounter,
                       target.comparatorType);
  }
  ...
  case TargetType::AttachedActivePokemon: {
    ...
    return BoolCompare(card.attachMoveCounter ==
                           state.getCard(ps.getActive()).moveCounter,
                       target.comparatorType);
  }
  ...
  case TargetType::IsAttachedEnergy:
    ...
    return BoolCompare(!state.game->energyList.empty(), target.comparatorType);
  case TargetType::AttachedEnergyCount:
    state.getEnergies(card.playerIndex, ref, state.game->energyList);
    return Compare(static_cast<int>(state.game->energyList.size()), target.val,
                   target.comparatorType);
  ...
  case TargetType::IsAttachedEnergyType: {
    state.getEnergies(card.playerIndex, ref, state.game->energyList);
    EnergyType targetType = static_cast<EnergyType>(target.val);
    bool found = false;
  ...
  case TargetType::IsAttachedTool: {
    ...
    return BoolCompare(!tools.empty(), target.comparatorType);
  }
  ...
  case TargetType::IsAttachedToolOrSpecialEnergy: {
    auto tools = state.getAttachedToolRef(card);
    bool found = !tools.empty();
    if (!found) {
      state.getEnergyCards(ref, state.game->cardList);
      for (CardRef ref : state.game->cardList) {
        if (state.getCard(ref).getMaster().cardType ==
  ...
  case TargetType::NotContextCardAttachedPokemon: {
    ...
    const Card &contextCard = state.getCard(state.contextCard);
    return BoolCompare(contextCard.attachMoveCounter != card.moveCounter,
                       target.comparatorType);
  }
  ...
  case TargetType::EnergyTypeAttached: {
    ...
    return BoolCompare(
        MatchEnergyType(ei.type, static_cast<EnergyType>(target.val)),
        target.comparatorType);
  }
  ...
  case TargetType::Area:
    return BoolCompare(static_cast<int>(card.area) == target.val,
                       target.comparatorType);
  ...
  case TargetType::SameTypeEnemy: {
    ...
    auto inPlayEnemy =
        state.players.at(1 - card.playerIndex).getInPlayPokemon();
  ...
  case TargetType::SpecialCondition:
    ...
    return BoolCompare(state.players.at(card.playerIndex).isSpecialCondition(),
                       target.comparatorType);
  case TargetType::SpecialConditionOrDamaged:
    ...
    return BoolCompare(
        state.players.at(card.playerIndex).isSpecialCondition() ||
            card.damage > 0,
        target.comparatorType);
  case TargetType::Poison:
    ...
    return BoolCompare(state.players.at(card.playerIndex).isPoisoned(),
                       target.comparatorType);
  case TargetType::Burn:
    if (card.area != AreaType::Active) {
      return false;
    }
    return BoolCompare(state.players.at(card.playerIndex).isBurned(),
                       target.comparatorType);
  case TargetType::Confuse:
    if (card.area != AreaType::Active) {
      return false;
    }
    return BoolCompare(state.players.at(card.playerIndex).badStatus ==
                           BadStatusType::Confused,
                       target.comparatorType);
  case TargetType::PoisonOrBurn:
    ...
    return BoolCompare(state.players.at(card.playerIndex).isPoisoned() ||
                           state.players.at(card.playerIndex).isBurned(),
                       target.comparatorType);
  ...
  case TargetType::SameNameEnemyField: {
    bool found = false;
    for (RefPosition rp :
         state.players[1 - state.getCard(effectCard.card).playerIndex]
             .getInPlayPokemon()) {
...
inline bool IsTarget(const State &state, CardRef ref, const Target &target,
                     const AreaRef &effectCard) {
...
inline void AddIfTarget(const State &state, CardRef ref, const Target &target,
                        std::vector<AreaRef> &output,
                        const AreaRef &effectCard) {
...
inline void TargetList(const State &state, const Target &target,
                       std::vector<AreaRef> &output, AreaRef effectCard,
                       int effectOwner) {
  if (target.areas.empty()) {
    output.clear();
    return;
  }
  ...
  case AreaType::TriggerSubject:
    ...
    if (state.notMoved(state.triggerInfo.subject)) {
      AddIfTarget(state, state.triggerInfo.subject.card, target, output,
                  effectCard);
    }
    return;
  case AreaType::TriggerObject:
    ...
    if (state.notMoved(state.triggerInfo.object)) {
      AddIfTarget(state, state.triggerInfo.object.card, target, output,
                  effectCard);
    }
    return;
    ...
    for (int playerIndex : state.basicPlayerOrder()) {
      ...
      for (AreaType areaType : target.areas) {
        ...
        case AreaType::Deck:
          ...
          break;
        case AreaType::Hand:
          if (target.skipEnemyTarget &&
              playerIndex != state.getCard(effectCard.card).playerIndex) {
        ...
        case AreaType::Looking:
          ...
          break;
        default:
          Exception("unexpected area target " +
                    std::to_string(static_cast<int>(areaType)));
          break;
        }
      }
    }
