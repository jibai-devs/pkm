...
#include "mechanics/AddLog.h"
#include "mechanics/AddOption.h"
...
inline void ClampShort(short &var, int value) {
  constexpr int shortMin = -30000;
  constexpr int shortMax = 30000;
  const int val = var + value;
  var = std::clamp(val, shortMin, shortMax);
}

inline void SetYesNoSelect(State &state, SelectContext selectContext,
                           int selectPlayer) {
...
inline void AddDamage(State &state, CardRef ref, Card &card, int damage,
                      bool isAttackDamage, CardRef cause,
                      bool putDamageCounter = false,
                      const Attack *attack = nullptr) {
  if (damage > 0) {
    const int maxHp = state.getMaxHp(card);
    if (card.damage >= maxHp) {
      // すでにHP0
      card.damage += damage;
    } else {
      const Card &causeCard = state.getCard(cause);
      const bool isEnemyAttackDamage =
          (isAttackDamage && causeCard.playerIndex != card.playerIndex);
      ...
      if (card.damage >= maxHp) {
        ...
        if (isEnemyAttackDamage) {
          ...
          if (causeCard.basicPrizePlus1 &&
              card.getMaster().evolutionType == EvolutionType::Basic) {
      ...
      if (isEnemyAttackDamage) {
        ...
        if (card.area == AreaType::Active) {
          PullTrigger(state, TriggerType::DamagedEnemyAttackActive, ref, cause,
                      1);
          for (int i = static_cast<int>(state.delayTriggerStack.size()) - 1;
               i >= 0; i--) {
            TriggeredAbility ta = state.delayTriggerStack[i];
            if (ta.trigger.type == TriggerType::DamagedEnemyAttackActive &&
                ta.trigger.subject.card == ref) {
...
inline void AfterDamage(State &state, CardRef ref, Card &card, CardRef causeRef,
                        const Card &causeCard) {
  if (card.specialFlagTool) {
    const EnergyType attackerType = state.getEnergyType(causeCard);
    ...
    for (const AttachedToolEnergy &t : tools) {
      ...
      if (name == u8"ウタンのみ") {
        if (ContainsEnergyType(attackerType, EnergyType::Psychic)) {
          const int index = state.currentAreaIndex(ps.tool, t.ref);
          MoveCardBase(state, card.playerIndex, AreaType::Tool, index,
                       AreaType::Trash);
        }
...
inline int Heal(State &state, CardRef ref, int heal, bool isHeal) {
  ...
  const int srcDamage = card.damage;
  card.damage -= heal;
  if (card.damage < 0) {
    card.damage = 0;
  }
  const int healed = srcDamage - card.damage;
  if (healed >= 0) {
...
// ワザのダメージ
inline int CalcDamage(const State &state, int baseDamage, CardRef targetRef,
                      const Card &target, CardRef attackerRef,
                      const Card &attacker, bool calcWeakness,
                      const Attack *attack) {
  ...
  const EnergyType attackerType = state.getEnergyType(attacker);
  ...
  if (target.area == AreaType::Active &&
      attacker.playerIndex != target.playerIndex) {
    damage += attacker.damageChangeActive;
    damage += attacker.damageChangeThisTurn;
    damage += attacker.damageChangeEnemyTakenPrize *
              state.takenPrizeCount(1 - attacker.playerIndex);
  ...
  if (calcTargetEffect) {
    ...
    if (attacker.playerIndex != target.playerIndex) {
      ...
      if (target.noDamageEnemyAttack ||
          target.noDamageAndEffectEnemyAttackNextEnemyTurn) {
        damage = 0;
      }
    }
    if (target.noDamageAttackNextEnemyTurn ||
        target.noDamageAndEffectAttackNextEnemyTurn) {
      damage = 0;
    }
    ...
    if (target.noDamageBasicColorAttackNextEnemyTurn) {
      if (attackerMaster.evolutionType == EvolutionType::Basic &&
          attackerType != EnergyType::Colorless) {
        damage = 0;
      }
    }
  ...
  constexpr int maxDamage = 100'000'000;
