...
public:
  ...
  SearchInfo start(const SearchStartConfig &config, const State &state) {
    for (int id : config.enemyActive) {
      ...
      if (master.cardType != CardType::Pokemon &&
          !master.toBattleFieldOnlySetup && !master.toActiveOnlySetup) {
        return SearchInfo::error(2);
      }
    }
    if (IsActiveNull(state, 1 - state.selectPlayer) &&
        config.enemyActive.empty()) {
    ...
    int index = 3;
    auto set = [&](const std::vector<int> &ids, CardList &out, int playerIndex,
                   AreaType area) {
      for (int i : range(ids)) {
        int id = ids[i];
    ...
    auto setActive = [&]() {
      ...
      if (!active.empty() && active[0].isNull()) {
  ...
  int lastSearchId() const { return static_cast<int>(searchList.size()) - 1; }

  const State &lastState() const { return *searchList.back().state; }
