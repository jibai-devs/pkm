...
inline void MoveToBenchSelected(State &state, int playerIndex) {
  ...
  for (const AreaRef &ref : state.targetList) {
    const int handIndex = state.currentAreaIndex(ps.hand, ref.card);
    MoveListCard(ps.hand, ps.bench, handIndex);
    state.cardMoved(ref.card, AreaType::Bench, true);
    LogMoveCard(state, playerIndex, ref.card, AreaType::Hand, AreaType::Bench,
                true);
  }
}
...
inline void SelectedSetupBenchPokemon(State &state, int playerIndex) {
  if (playerIndex == state.firstPlayer) {
    ...
    Card &card1 =
        state.getCard(state.players[1 - state.firstPlayer].getActive());
...
inline void SetupBenchPokemon(State &state, int playerIndex) {
  state.setSelect(SelectType::Card, SelectContext::SetupBenchPokemon,
                  playerIndex);
  ...
  state.selectMin = 0;
  state.selectMax = std::min(static_cast<int>(state.options.size()),
                             state.remainingBench(playerIndex));
...
inline void StartSetupBench(State &state) {
  ...
  for (int i : state.basicPlayerOrder()) {
    const int mulliganCount = state.mulliganCount[i];
    if (mulliganCount > 0) {
      const int selectPlayer = 1 - i;
      state.setSelect(SelectType::Count, SelectContext::DrawCount,
                      selectPlayer);
...
inline void SelectedSetupActivePokemon(State &state, int playerIndex) {
  const SelectOption option = state.firstSelected();
  state.clearSelect();
  PlayerState &ps = state.players[playerIndex];
  const int handIndex = option.getCardPosition().areaIndex;
  const CardRef ref = MoveListCard(ps.hand, ps.active, handIndex);
...
inline void SetupActivePokemon(State &state, int playerIndex) {
  ...
  if (state.mulligan[playerIndex]) {
    ...
    state.setSelect(SelectType::Card, SelectContext::SetupActivePokemon,
                    playerIndex);
    auto &hand = state.players[playerIndex].hand;
    for (int i : range(hand)) {
...
inline void SelectedIsFirst(State &state) {
  const int selectPlayer = state.selectPlayer;
