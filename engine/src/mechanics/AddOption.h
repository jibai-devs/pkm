...
inline void AddOptionCard(State &state, AreaType area, int index,
                          int playerIndex) {
  SelectOption &option = state.addOption(SelectOptionType::Card);
  option.param0 = static_cast<short>(area);
  option.param1 = index;
  option.param2 = playerIndex;
}

inline void AddOptionToolCard(State &state, AreaType area, int index,
                              int playerIndex, int toolIndex) {
...
inline void AddOptionEnergyCard(State &state, AreaType area, int index,
                                int playerIndex, int energyIndex) {
...
inline void AddOptionEnergy(State &state, AreaType area, int index,
                            int playerIndex, int energyIndex, int count) {
...
inline void AddOptionAttach(State &state, AreaType area, int index,
                            AreaType inPlayArea, int inPlayIndex) {
  SelectOption &option = state.addOption(SelectOptionType::Attach);
  option.param0 = static_cast<short>(area);
  option.param1 = index;
  option.param2 = static_cast<short>(inPlayArea);
  option.param3 = inPlayIndex;
}

inline void AddOptionEvolve(State &state, AreaType area, int index,
                            AreaType inPlayArea, int inPlayIndex) {
...
inline void AddOptionAbility(State &state, AreaType area, int index) {
  assert(area == AreaType::Active || area == AreaType::Bench ||
         area == AreaType::Stadium);
...
inline void AddOptionAttack(State &state, int attackId, int srcAttackId,
                            int benchIndex = -1) {
