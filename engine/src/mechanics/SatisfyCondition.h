...
#include "mechanics/TargetList.h"

inline bool SatisfyCondition(const State &state,
                             const std::vector<Effect> &effects,
                             int effectIndex, CardRef effectCard,
                             int effectOwner) {
  ...
  case ConditionType::AnyTargetAfterEffect: {
    ...
    for (int i = effectIndex + 1; i < std::ssize(effects); i++) {
      ...
      if (!e.isCondition) {
        TargetList(state, e.target, targetList, state.makeAreaRef(effectCard),
                   effectOwner);
        if (!targetList.empty()) {
          found = true;
          break;
  ...
  case ConditionType::CountTarget: {
    std::vector<AreaRef> &targetList = state.game->targetList;
    TargetList(state, effect.target, targetList, state.makeAreaRef(effectCard),
               effectOwner);
    return Compare(static_cast<int>(targetList.size()), effect.values[0],
                   effect.comparatorType);
  }
  case ConditionType::CountTarget2: {
    std::vector<AreaRef> &targetList = state.game->targetList;
    TargetList(state, effect.target, targetList, state.makeAreaRef(effectCard),
               effectOwner);
    const int count = static_cast<int>(targetList.size());

    return BoolCompare(effect.values[0] == count || effect.values[1] == count,
                       effect.comparatorType);
  }
  case ConditionType::CountTargetMeOrEnemy: {
    ...
    TargetList(state, target, targetList, state.makeAreaRef(effectCard),
               effectOwner);
    int countMe = static_cast<int>(targetList.size());

    target.targetPlayer = TargetPlayer::Enemy;
    TargetList(state, target, targetList, state.makeAreaRef(effectCard),
               effectOwner);
    const int countEnemy = static_cast<int>(targetList.size());
  ...
  case ConditionType::CompareCountTargetMeEnemy: {
    ...
    return Compare(counts[effectOwner], counts[1 - effectOwner],
                   effect.comparatorType);
  }
  case ConditionType::CountEnergy: {
    int count = 0;
    for (int i : state.effectTargetPlayerContinual(effect, effectOwner)) {
      const auto inPlay = state.players[i].getInPlayPokemon();
  ...
  case ConditionType::CountEnergyType: {
    const EnergyType type =
        static_cast<EnergyType>(effect.target.conditions.at(0).val);
    int count = 0;
    for (int i : state.effectTargetPlayerContinual(effect, effectOwner)) {
      const auto inPlay = state.players[i].getInPlayPokemon();
      for (const RefPosition &p : inPlay) {
  ...
  case ConditionType::AttackEnergyExtra: {
    ...
    state.getEnergies(attacker.playerIndex, state.attacker,
                      state.game->energyList);
    int extra = InsufficientEnergyCount(
        state, attack.energies, state.game->energyList, attacker, attack, true);
    return Compare(-extra, effect.values[0], effect.comparatorType);
  }
  case ConditionType::NotFullBench: {
    int playerIndex =
        (effect.target.targetPlayer == TargetPlayer::Me ? effectOwner
                                                        : (1 - effectOwner));
    return BoolCompare(state.remainingBench(playerIndex) >= 1,
                       effect.comparatorType);
  }
  case ConditionType::MyTurn:
    if (state.isPlayerTurn()) {
      const Card &card = state.getCard(effectCard);
      return BoolCompare(card.playerIndex == state.activePlayerIndex(),
                         effect.comparatorType);
    } else {
      return false;
    }
  ...
  case ConditionType::KoPreEnemyTurnTeamRocket:
    return BoolCompare(state.turnHistories[1].koTeamRocket,
                       effect.comparatorType);
  case ConditionType::KoAttackDamagePreEnemyTurn:
    return BoolCompare(state.turnHistories[1].koAttackDamage,
                       effect.comparatorType);
  case ConditionType::KoAttackDamageEthanPreEnemyTurn:
    return BoolCompare(state.turnHistories[1].koAttackDamageEthan,
                       effect.comparatorType);
  case ConditionType::KoAttackDamageHopPreEnemyTurn:
    return BoolCompare(state.turnHistories[1].koAttackDamageHop,
                       effect.comparatorType);
  ...
  case ConditionType::SameAttackPreMyTurn:
    return BoolCompare(
        state.turnHistories[2].turnAttackId == state.currentAttackId &&
            state.turnHistories[2].turnAttackCard == state.attacker,
        effect.comparatorType);
  case ConditionType::CoinHeadCount:
    return Compare(state.coinHeadCount, effect.values[0],
                   effect.comparatorType);
  ...
  case ConditionType::MysteryGarden: {
    ...
    const auto inPlay = ps.getInPlayPokemon();
    for (const RefPosition &rp : inPlay) {
...
inline bool SatisfySkillCondition(const State &state, const Skill &skill,
                                  CardRef effectCard, int effectOwner,
                                  int startIndex = 0) {
...
inline bool CanPlaySkill(const State &state, CardRef ref, const Card &card,
...
inline bool CanActivateMainAbility(const State &state, CardRef ref,
                                   int playerIndex) {
...
inline bool SatisfyAttackCondition(const State &state, const Attack &attack,
                                   CardRef attackerRef, const Card &attacker) {
  if (!attack.preEffects.empty() && !attack.postEffects.empty()) {
    return true;
  }
  for (int i : range(attack.postEffects)) {
    ...
    if (!SatisfyCondition(state, attack.postEffects, i, attackerRef,
