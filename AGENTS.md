...
7. **Multi-deck training** — all 25 real opponent decklists are sourced (Part 3a), 25 solo pool bots are trained (Part 3b, 65-100% eval-vs-random), cross-archetype sampling is implemented and smoke-tested (Part 3c, `pkm train --archetype-pool`), the classifier's belief is wired into training (`--archetype-belief`), and the `03_pult_munki` full retrain using both is in progress (Milestone 8, above). **Reopened same day:** simultaneous population training (`pkm/rl/population_train.py`, `PopulationMember`/`PopSpec`) is no longer just a documented fallback — it's the explicit plan for the *next* `03_pult_munki` training run after Milestone 8, regardless of how that retrain turns out. Still unbuilt; see `docs/opponent-archetype-classifier-plan.md` Part 3 §3b+3c (Milestone 9) for the design to implement.
