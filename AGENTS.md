...
7. **Multi-deck training** — all 25 real opponent decklists are sourced (Part 3a), 25 solo pool bots are trained (Part 3b, 65-100% eval-vs-random), cross-archetype sampling is implemented and smoke-tested (Part 3c, `pkm train --archetype-pool`), and the classifier's belief is now actually wired into training (`--archetype-belief`, not just unit-tested at the `TorchPolicy` level). Next: run the full `03_pult_munki` retrain with **both** flags on (Milestone 8) — needs a fresh run, since its existing checkpoint can't be resumed from (belief-feature-resize breakage above). Simultaneous population training (`pkm/rl/population_train.py`, `PopulationMember`/`PopSpec`) remains a documented fallback-of-the-fallback if this frozen-pool retrain underperforms — see `docs/opponent-archetype-classifier-plan.md` Part 3.
