"""Tests for the combination-scoring policy head (policy_head="combo").

Guarantees (mirrors test_autoreg_head.py):
  ...
  * the [B,L] head keeps its meaning (the combo distribution marginalized to
    per-option inclusion logits) so MCTS `evaluate` / ExIt / inference-MCTS work
    unchanged, and for k==1 it reduces to the combo categorical over options;
  * sampling respects the count bounds (minCount<=m<=maxCount, distinct, valid)
    and the empty set is reachable iff minCount==0;
  * a sampled combo's returned logprob equals its batched recompute (the core
    consistency invariant the trainer relies on), matched order-independently;
  * gradients flow into the combo head; degenerate rows never NaN;
  * end-to-end: PPO rollout+update, ExIt update, and a full inference game.
"""
...
from pkm.new_agents.agent_000_dragapult.model import collate, enumerate_combos
...
def _combo_model():
    torch.manual_seed(0)
    return build_model(ModelConfig(policy_head="combo")).eval()
...
def test_default_is_marginal() -> None:
    ...
    assert hasattr(m, "scorer") and not hasattr(m, "combo")


def test_combo_builds_the_head_only() -> None:
    m = build_model(ModelConfig(policy_head="combo"))
    assert m.policy_head == "combo"
    assert hasattr(m, "combo")
    assert not hasattr(m, "scorer") and not hasattr(m, "autoreg")
...
def test_config_hash_differs_and_backfills() -> None:
    ...
    assert len({marg.hash(), combo.hash(), auto.hash()}) == 3
...
def test_enumerate_counts_and_empty() -> None:
    # k==1: n options -> n singleton combos, no empty set.
    b = collate([_feats(4, 1, 1)])
    idx, mm, valid = enumerate_combos(b)
    n_valid = int(valid[0].sum())
    assert n_valid == 4
    assert int(mm[0].sum()) == 4  # each combo has exactly one member
    # minCount==0, maxCount==2: sizes 0,1,2 -> 1 + 3 + 3 = 7 combos, incl. empty.
    b = collate([_feats(3, 0, 2)])
    idx, mm, valid = enumerate_combos(b)
    assert int(valid[0].sum()) == 7
    sizes = mm[0][valid[0].bool()].sum(-1)  # members per real combo
    assert (sizes == 0).any()  # the empty set is present


def test_enumerate_caps_at_64() -> None:
    # C(12, 6) = 924 size-6 combos -> hard-capped at 64.
    b = collate([_feats(12, 6, 6)])
    _idx, _mm, valid = enumerate_combos(b)
    assert int(valid[0].sum()) == 64


# --------------------------------------------------------------------------- #
# [B,L] head keeps its meaning -> MCTS/ExIt/inference untouched
# --------------------------------------------------------------------------- #
...
def test_k1_marginal_matches_combo_categorical() -> None:
    # For k==1 each combo is a single option, so the marginalized [B,L] softmax
    # equals the combo categorical over options (each option = its own combo).
    m = _combo_model()
    b = collate([_feats(5, 1, 1)])
    with torch.no_grad():
        state, ent = m.encode(b)
        priors = torch.softmax(m.policy_from_state(state, ent, b), dim=-1)[0]
        combo_logits, combo_idx, mm, _valid = m.policy_combos(state, ent, b)
    combo_p = torch.softmax(combo_logits[0], dim=-1)
    # map each singleton combo's prob to its option
    mapped = torch.zeros(5)
    for c in range(combo_idx.shape[1]):
        member = int(combo_idx[0, c, 0])
        if mm[0, c, 0] > 0:
            mapped[member] += float(combo_p[c])
    assert torch.allclose(priors, mapped, atol=1e-5)


def test_policy_combos_rejected_on_marginal() -> None:
    import pytest
    ...
    with pytest.raises(RuntimeError):
        m.policy_combos(state, ent, b)


# --------------------------------------------------------------------------- #
# Sampling respects count bounds
# --------------------------------------------------------------------------- #
...
def test_sample_count_bounds_and_distinct() -> None:
    ...
    for _ in range(200):
        picks, _ = policy.sample_action_combo(m, state, ent, b, gen=gen)
...
def test_min_count_zero_allows_empty() -> None:
    ...
    lengths = {
        len(policy.sample_action_combo(m, state, ent, b, gen=gen)[0])
        for _ in range(300)
    }
...
def _recompute(m, state, ent, b, picks):
    ...
    with torch.no_grad():
        return float(
            policy.batched_action_logprob_combo(m, state, ent, b, actions, action_len)[0]
        )
...
def test_sample_logprob_matches_recompute() -> None:
    ...
    for (n, lo, hi), seed in [((5, 1, 3), 1), ((4, 0, 2), 7), ((6, 2, 4), 9), ((3, 1, 1), 2)]:
        ...
        for _ in range(25):
            picks, lp = policy.sample_action_combo(m, state, ent, b, gen=gen)
            recomputed = _recompute(m, state, ent, b, picks)
            assert abs(lp - recomputed) < 1e-4, (n, lo, hi, picks, lp, recomputed)


def test_recompute_is_order_independent() -> None:
    # The combo is a *set*; a permuted action must recompute to the same logprob.
    m = _combo_model()
    b = collate([_feats(5, 2, 3)])
    state, ent = m.encode(b)
    lp_a = _recompute(m, state, ent, b, [1, 3])
    lp_b = _recompute(m, state, ent, b, [3, 1])
    assert abs(lp_a - lp_b) < 1e-6
...
def test_batched_logprob_matches_per_row() -> None:
    ...
    with torch.no_grad():
        batched = policy.batched_action_logprob_combo(m, state, ent, b, actions, action_len)
...
def test_gradient_flows_into_combo_head() -> None:
    torch.manual_seed(0)
    m = build_model(ModelConfig(policy_head="combo")).train()
    b = collate([_feats(4, 1, 2)])
    state, ent = m.encode(b)
    actions = torch.tensor([[1, 3]], dtype=torch.long)
    action_len = torch.tensor([2], dtype=torch.long)
    lp = policy.batched_action_logprob_combo(m, state, ent, b, actions, action_len)
    lp.sum().backward()
    assert m.combo.scorer[0].weight.grad is not None
    assert m.combo.scorer[2].weight.grad is not None
...
def test_entropy_finite_and_positive() -> None:
    ...
    state, ent = m.encode(b)
    h = policy.batched_entropy_combo(m, state, ent, b)
    assert h.shape == (2,)
    assert torch.isfinite(h).all() and (h >= -1e-6).all()
...
def _combo_cfg():
    return Config(model=ModelConfig(policy_head="combo"))


def test_ppo_rollout_update_smoke_combo() -> None:
    ...
    # A real self-play game hits multi-select nodes, so the count must vary.
...
def test_exit_update_smoke_combo() -> None:
    # ExIt consumes only the [B,L] head, so it must work unchanged with combo.
...
def test_inference_agent_plays_full_game_combo() -> None:
    ...
    while obs["current"]["result"] < 0 and n_iter < 100000:
        picks = agent(obs)  # exercises the combination inference sampler
