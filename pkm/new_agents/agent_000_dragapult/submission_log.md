...
|---|---|---|---|---|---|
| 2026-07-24 00:33 | `015_alakazam_exit_combo_large/ckpt_768.pt` (large, combo head; run still training) | policy (`--inference policy`) | `submission_20260724_003257.tar.gz` | 015 alakazam ExIt combo large ckpt_768, POLICY | _blocked: HTTP 400 (daily 5-submission cap for 2026-07-23 UTC). Bundle packed & on disk; auto-resubmit scheduled for UTC-midnight reset (~08:00 local 07-24), cron job `0d9ccf51`_ |
| 2026-07-24 00:32 | `010_alakazam_exit_tdlambda_medium/ckpt_4608.pt` (medium, marginal head) | policy (`--inference policy`) | `submission_20260724_003238.tar.gz` | 010 alakazam ExIt TD(lambda) medium ckpt_4608, POLICY (--inference policy) (ref 54932458) | _pending — duplicate of the 723.4 run below (identical net, `mcts K=0` ≡ `policy`)_ |
