...
# --- LOCAL ADAPTATION -----------------------------------------------------
# Instead of the Kaggle `cg-lib` dataset, drive the SAME libcg through this
# repo's engine seam. `cabt` re-implements the whole `cg.api`/`cg.game` surface
# the notebook needs (typed Observation/Pokemon/Option enums + battle_*/search_*
# + all_card_data/all_attack). The only renames vs the Kaggle API:
#   * `to_observation_class`  -> `cabt.to_observation`
#   * `search_begin` takes the RAW obs dict (not the typed Observation); the
#     one call site below passes `obs_dict` accordingly.
from pkm.new_agents.agent_000_dragapult.cabt import (  # noqa: E402
    AreaType,
    Card,
    Observation,
    OptionType,
...
SEARCH_COUNT = int(os.environ.get("SEARCH_COUNT", "10")) # MCTS Search count
# LOCAL: env knobs so this doubles as a fast smoke test.
N_ITERS = int(os.environ.get("N_ITERS", "5"))       # outer train iterations
N_EVAL = int(os.environ.get("N_EVAL", "50"))        # eval games / iter
N_SELFPLAY = int(os.environ.get("N_SELFPLAY", "100")) # self-play games / iter
...
for counter in range(N_ITERS):
    ...
    with torch.inference_mode():
        ...
        # Self Play
        for _ in progress(N_SELFPLAY, "Training Data Collecting... "):
