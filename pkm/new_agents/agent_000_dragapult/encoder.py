...
from pkm.agents.agent_000_dragapult.cards import A, build_card_attr_table
from pkm.agents.agent_000_dragapult.features import F, G, Features, _VOCAB
...
class StateEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.entity_proj = nn.Sequential(
            nn.Linear(d_card + F, d_entity), nn.ReLU()
        )
        ...
        self.global_mlp = nn.Sequential(
            nn.Linear(G + 2 * _VOCAB, d_global), nn.ReLU()
        )
        self.head = nn.Sequential(
            nn.Linear(d_entity + d_global, d_state), nn.ReLU()
        )
